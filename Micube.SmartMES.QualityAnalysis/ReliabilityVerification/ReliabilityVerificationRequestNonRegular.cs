...
namespace Micube.SmartMES.QualityAnalysis
{
    ...
    /// 
    /// </summary>
    public partial class ReliabilityVerificationRequestNonRegular : SmartConditionManualBaseForm
    {
        ...
        /// 생성자
        /// </summary>
        public ReliabilityVerificationRequestNonRegular()
        {
            InitializeComponent();
        }
        ...
        {
            //ReliabilityVerificationRequestNonRegularPopup popup = new ReliabilityVerificationRequestNonRegularPopup(grdReliabiVerifiReqRgistRegular.View.GetFocusedDataRow());
            //popup.ShowDialog();

            var values = Conditions.GetValues();
            string sPlant = values["P_PLANTID"].ToString();

            ReliabilityVerificationRequestNonRegularPopup popup = new ReliabilityVerificationRequestNonRegularPopup("New", UserInfo.Current.Enterprise, UserInfo.Current.Plant ,"" ,"");
        ...
        private void GrdReliabiVerifiReqRgistRegularView_DoubleClick(object sender, EventArgs e)
        {
            ...
            ReliabilityVerificationRequestNonRegularPopup popup = new ReliabilityVerificationRequestNonRegularPopup("Modify", row["ENTERPRISEID"].ToString(), row["PLANTID"].ToString(), row["REQUESTNO"].ToString(), row["ISSAMPLERECEIVE"].ToString());
            popup.ShowDialog();
        ...
        protected async override Task OnSearchAsync()
        {
            ...
            var values = Conditions.GetValues();
            values.Add("P_RELIABILITYTYPE", "NonRegular");
            DataTable dt = null;

            dt = await SqlExecuter.QueryAsync("GetReliabilityVerificationRequestNonRegularRgisterList", "10001", values);
