...
{
    /// <summary>
    /// 프 로 그 램 명  : 품질관리 > 출하검사 > 3D 측정값 상세 팝업
    /// 업  무  설  명  : 3D 설비 인퍼페이스 측정값 Popup 
    /// 생    성    자  : 전우성
    /// 생    성    일  : 2020-03-03
    /// 수  정  이  력  : 
    /// 
    /// 
    /// </summary>
    public partial class popupThreeDimensionMeasure : SmartPopupBaseForm, ISmartCustomPopup
    {
        #region Interface
        ...
        public popupThreeDimensionMeasure()
        {
            InitializeComponent();
        ...
        private void InitializeLanguageKey()
        {
            ...
            layoutInfo.SetLanguageKey(layoutItemId, "MEASUREITEM");
        ...
        private void InitializeGrid(int maxPoint)
        {
            ...
            grdMain.View.AddTextBoxColumn("EQUIPMENTPRODUCTID", 70).SetTextAlignment(TextAlignment.Right);
        ...
        public void InitializeEvent()
        {
            ...
            this.Load += (s, e) =>
            {
                ...
                txtItemId.Text = DefectMapHelper.StringByDataRowObejct(CurrentDataRow, "DAITEMID");
        ...
        private void GetMeasureValueList(DataRow dr)
        {
            Dictionary<string, object> param = new Dictionary<string, object>()
            {
                ...
                {"P_DAITEMID", dr["DAITEMID"] }
            };

            if (SqlExecuter.Query("Get3DMeasureByLot", "10001", param) is DataTable dt)
            {
                if (dt.AsEnumerable()
                      .Where(x => x.Field<decimal>("DEGREE") == Format.GetDecimal(dr["DEGREE"]))
                      .Max(x => Format.GetInteger(x.Field<string>("PIECEID"))) is int maxPoint)
                {
                    ...
                    {
                        if (DefectMapHelper.IntByDataRowObject(cloneDr, "PIECEID") > defaultNo)
                        {
                            newRow["DEGREE"] = DefectMapHelper.StringByDataRowObejct(cloneDr, "DEGREE");
                            newRow["EQUIPMENTPRODUCTID"] = DefectMapHelper.StringByDataRowObejct(cloneDr, "EQUIPMENTPRODUCTID");
                            defaultNo = DefectMapHelper.IntByDataRowObject(cloneDr, "PIECEID");
