...
{
    public partial class FileProgressDialogPopup : SmartPopupBaseForm
    {
        // File List
        ...
        public MeasureProgressingResult Result = new MeasureProgressingResult();

        public FileProgressDialogPopup(object dataSource, UpDownType upDownType, string folderPath, int totalFileSize)
        {
            InitializeComponent();
        ...
        private void InitializeGrid()
        {
            ...
            if (_dataSource != null && (_dataSource as DataTable).Rows.Count > 0 && _upDownType.Equals("Upload"))
                FileUploadToServer();
        }
                ...
                foreach (DataRow row in dataSource.Rows)
                {
                    ...
                    if (row["PROCESSINGSTATUS"].Equals("Downloading"))
                    {
                        try
                        {
                            ...
                            string fileName = Format.GetString(row["FILENAME"]);
                            string fileExt = Format.GetString(row["FILEEXT"]);
                            ...
                            using (WebClient client = new WebClient())
                            {
                                ...
                                if (file.Length != 0)
                                {
                                    ...
                                    {
                                        if (file[i].Name.Equals(safeFileName))
                                        {
                                            client.DownloadFile(serverPath, _folderPath + "\\" + fileName + today + "." + fileExt);
                                            break;
                                        }

                                        if (i == file.Length - 1) client.DownloadFile(serverPath, string.Join("\\", _folderPath, Format.GetString(row["SAFEFILENAME"])));
                                    }
                                }
                                else
                                {
                ...
                foreach (DataRow row in dataSource.Rows)
                {
                    ...
                    if (row["PROCESSINGSTATUS"].Equals("Downloading"))
                    {
                        ...
                        try
                        {
                            ...
                            if (file.Length != 0)
                            {
                                for (int i = 0; i < file.Length; i++)
                                {
                                    if (file[i].Name.Equals(safeFileName))
                                    {
                                        ...
                                        Client.DownloadFile(serverPath + "\\" + safeFileName, @downloadPath + "\\" + fileName + today + "." + fileExt);
                                        break;
                                    }
                                }
                            }
                            ...
                            {
                                DeployCommonFunction.DownLoadFile(serverPath, filePath, safeFileName);
                            }
                        }
                        catch (Exception ex)
                        {
