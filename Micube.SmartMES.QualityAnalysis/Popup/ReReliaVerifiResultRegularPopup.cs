...
namespace Micube.SmartMES.QualityAnalysis
{
    ...
    /// 
    /// </summary>
    public partial class ReReliaVerifiResultRegularPopup : SmartPopupBaseForm, ISmartCustomPopup
    {
        ...
        public ReReliaVerifiResultRegularPopup(DataRow dr)
        {
            InitializeComponent();
        ...
        private void InitializeLanguageKey()
        {
            ...
            btnImageSave1.LanguageKey = "EXPORT";
        }
        ...
        private void InitializeGrid()
        {
            ...
            InitializeGrdPrvProductInformation();  // 제품 정보 그리드 초기화(이전결과)

            InitializeGrdPrvMeasureHistory(); // 제조 이력 그리드 초기화(이전결과)

            InitializeGridMeasureValue(); // 측정값 입력 그리드 초기화
            InitializeGridPrvMeasureValue(); // 측정값 입력 그리드 초기화(이전결과)
        ...
        {
            //grdSecondOpinion.View.GridMultiSelectionMode = GridMultiSelectionMode.CheckBoxSelect;
        ...
        /// 제품 정보 그리드 초기화(이전결과)
        /// </summary>
        private void InitializeGrdPrvProductInformation()
        {
            grdPrvProductnformation.GridButtonItem = GridButtonItem.Expand | GridButtonItem.Restore;

            grdPrvProductnformation.View
                .AddTextBoxColumn("PRODUCTDEFID", 150)
                .SetIsReadOnly()
                .SetTextAlignment(TextAlignment.Left); // 품목코드
            ...
            grdPrvProductnformation.View.PopulateColumns();
        }

        /// <summary>
        /// 제조 이력 그리드 초기화(이전결과)
        /// </summary>
        private void InitializeGrdPrvMeasureHistory()
        {
            grdPrvManufacturingHistory.GridButtonItem = GridButtonItem.Expand | GridButtonItem.Restore;

            grdPrvManufacturingHistory.View
            ...
            grdPrvManufacturingHistory.View.PopulateColumns();
        }
        ...
        private void InitializeGridMeasureValue()
        {
            ...
            grdMeasureValue.View
                .AddTextBoxColumn("MEASUREVALUE", 210)
        ...
        /// 측정값 그리드 초기화(이전결과)
        /// </summary>
        private void InitializeGridPrvMeasureValue()
        {
            grdPrvMeasureValue.GridButtonItem = GridButtonItem.Export;

            grdPrvMeasureValue.View.SetIsReadOnly();

            grdPrvMeasureValue.View
                .AddTextBoxColumn("TITLE", 210)
                .SetTextAlignment(TextAlignment.Left); // 제목
            ...
            grdPrvMeasureValue.View.PopulateColumns();
            ...
            grdPrvMeasureValue.View.Columns["MEASUREVALUE"].ColumnEdit = edit;
            */
        }
        ...
        private void InitializeEvent()
        {
            ...
            btnPrvClose.Click += BtnClose_Click;
            ...
            // 이미지 저장 이벤트(이전결과)
            btnImageSave1.Click += (s, e) =>
            {
                var enumerator = flowMeasuredPicture1.Controls.GetEnumerator();
                var i = 0;
                ...
                try
                {
                    ...
                    if (dialog.ShowDialog().Equals(DialogResult.OK))
                    {
                        ...
                        enumerator = flowMeasuredPicture1.Controls.GetEnumerator();
                        ...
                        while (enumerator.MoveNext())
                        {
                            ...
                            {
                                Bitmap image = (Bitmap)vr.selectImage();
                                image.Save(string.Concat(folderPath, "\\", vr.strFileName()));
                            }
                        }
            ...
            if (tabMeasuredValue.SelectedTabPageIndex == 0)
            {
                if (e.KeyCode == Keys.V && e.Modifiers == Keys.Control)
                {
                    ...
                    else if ((ClipboardFormat)format == ClipboardFormat.Bitmap)
                    {
                        ...
                        ReqularMultiVerificationResultControl vr = new ReqularMultiVerificationResultControl(image, sBitmapName);
                        flowMeasuredPicture.Controls.Add(vr);
        ...
        private void ReliaVerifiResultRegularPopup_Load(object sender, EventArgs e)
        {
            ...
            grdPrvAttachedFile.LanguageKey = "ATTACHEDFILE";
            grdPrvReport.LanguageKey = "REPORT";

            grdPrvAttachedFile.ButtonVisible = false;

            SetMainInfo(); // 기본 입력 정보 조회

            SearchInspectionFile(); // 검증 결과 조회
            SearchPrvInspectionFile(); // 검증 결과 조회(이전결과)
            SearchProductInformation(); // 제품 정보 조회
            SearchPrvProductInformation();  // 제품 정보 조회(이전결과)        
            SearchMeasuredValue(); // 측정값
                ...
                foreach (DataRow dr in _dtInspectionFile.Rows)
                {
                    ...
                    if (file != null)
                    {
                        try
                        {
                            ...
                            ReqularMultiVerificationResultControl vr = new ReqularMultiVerificationResultControl(image, fileName);
                            vr.setTitle(title);
        ...
        /// 검증 결과 조회(이전결과)
        /// </summary>
        private void SearchPrvInspectionFile()
        {
            Dictionary<string, object> param = new Dictionary<string, object>();
            param.Add("P_INSPECTIONTYPE", "ReliaVerifiResultRegular");
            param.Add("P_RESOURCEID", CurrentDataRow["LOTID"].ToString());//품목 + LOT
            param.Add("P_PROCESSRELNO", CurrentDataRow["PARENTREQUESTNO"].ToString());
            param.Add("P_MEASURETYPE", "VerificationResult");
            _dtInspectionFile = SqlExecuter.Query("SelectInspectionFile", "10001", param);
            ...
            if (_dtInspectionFile != null && _dtInspectionFile.Rows.Count != 0)
            {
                ...
                foreach (DataRow dr in _dtInspectionFile.Rows)
                {
                    ...
                    if (!string.IsNullOrEmpty(file.ToString()))
                    {
                        ...
                        flowMeasuredPicture1.Controls.Add(vr);
                    }
                }
            }
        }
        ...
        /// 기본 입력 정보 설정(이전결과)
        /// </summary>
        private void SetPrvMainInfo(DataRow dr)
        {
            txtPrvJudgmentResult.EditValue = dr["INSPECTIONRESULT"]; // 판정결과
            txtPrvIsNCRPublish.EditValue = dr["ISNCRPUBLISH"]; // NCR발행여부
            txtPrvDefectCodeName.EditValue = dr["DEFECTCODENAME"]; // 불량명
            txtPrvIsCompletion.EditValue = dr["ISCOMPLETION"]; // 완료여부
            memPrvComments.EditValue = dr["COMMENTS"]; // 특이사항
            txtQcSegmentName1.Text = dr["QCSEGMENTNAME"].ToString(); // 품질공정 명
            txtInspItemName.Text = dr["INSPITEMNAME"].ToString(); // 검증항목 명
            txtInspector.Text = dr["INSPECTOR"].ToString(); // 검사자
        }
        ...
        /// 제품 정보 조회(이전결과)
        /// </summary>
        private void SearchPrvProductInformation()
        {
            Dictionary<string, object> values = new Dictionary<string, object>();
            ...
            grdPrvProductnformation.DataSource = dt;

            SetPrvMainInfo(dt.Rows[0]); // 기본 입력 정보 설정(이전결과)
            SearcPrvhMeasuringHistory(dt.Rows[0]); // 제조 이력 조회(이전결과)
            SearchPrvMeasuredValue(dt.Rows[0]); // 측정값(이전결과)
            SearchPrvAttachFile(dt.Rows[0]); // 첨부파일(이전결과)
            SearchPrvReport(dt.Rows[0]); // 보고서(이전결과)
        }

        /// <summary>
        /// 제조 이력 정보 조회(이전결과)
        /// </summary>
        private void SearcPrvhMeasuringHistory(DataRow dr)
        {
            Dictionary<string, object> values = new Dictionary<string, object>();
            values.Add("P_REQUESTNO", dr["REQUESTNO"]);
            values.Add("P_LANGUAGETYPE", UserInfo.Current.LanguageType);

            DataTable dt = SqlExecuter.Query("GetManufacturingHistoryList", "10001", values);

            grdPrvManufacturingHistory.DataSource = dt;
        }
        ...
        /// 측정값 조회(이전결과)
        /// </summary>
        private void SearchPrvMeasuredValue(DataRow dr)
        {
            Dictionary<string, object> values = new Dictionary<string, object>();
            values.Add("P_REQUESTNO", dr["REQUESTNO"].ToString());
            values.Add("P_ENTERPRISEID", dr["ENTERPRISEID"].ToString());
            values.Add("P_PLANTID", dr["PLANTID"].ToString());
            values.Add("P_LOTID", dr["LOTID"].ToString());
            //values.Add("P_LOTWORKTXNHISTKEY", dr["TXNHISTKEY"].ToString());
            values.Add("P_MEASURETYPE", "MeasuredValue");
            ...
            if (dt.Rows.Count > 0)
            {
                cboPrvUnit.EditValue = dt.Rows[0]["UNIT"]; // 단위 설정
            }

            grdPrvMeasureValue.DataSource = dt;
        }
        ...
        /// 첨부파일 조회(이전결과)
        /// </summary>
        private void SearchPrvAttachFile(DataRow dr)
        {
            Dictionary<string, object> values = new Dictionary<string, object>();
            values.Add("P_FILERESOURCETYPE", "ReliaVerifiResultRegularAttach");
            values.Add("P_FILERESOURCEID", dr["REQUESTNO"].ToString() + dr["LOTID"].ToString());
            values.Add("P_FILERESOURCEVERSION", "1");

            DataTable dt = SqlExecuter.Query("SelectFileInspResult", "10001", values);

            grdPrvAttachedFile.DataSource = dt;
        }
        ...
        /// 보고서 조회(이전결과)
        /// </summary>
        private void SearchPrvReport(DataRow dr)
        {
            Dictionary<string, object> values = new Dictionary<string, object>();
            values.Add("P_FILERESOURCETYPE", "ReliaVerifiResultRegularReport");
            values.Add("P_FILERESOURCEID", dr["REQUESTNO"].ToString() + dr["LOTID"].ToString());
        ...
        private void selectUnitList()
        {
            ...
            DataTable unitDt = SqlExecuter.Query("GetUOMList", "10001", values);

            cboUnit.DataSource = unitDt; // 단위
            cboPrvUnit.DataSource = unitDt; // 단위(이전결과)
        }
        ...
        private void txtInspItemId_Load(object sender, EventArgs e)
        {
