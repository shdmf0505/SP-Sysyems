...
{
    /// <summary>
    /// 프 로 그 램 명  : 공정 관리 > 공정작업 > 인수 취소
    /// 업  무  설  명  : 다중 Lot을 선택하여 일괄 인수 취소 처리 한다.
    /// 생    성    자  : 강호윤
    /// 생    성    일  : 2019-09-09
    /// 수  정  이  력  : 
    /// 
    /// 
    /// </summary>
    public partial class LotAcceptCancel : SmartConditionManualBaseForm
    {
        ...
        private void InitializeGrid()
        {
            ...
            grdLotList.View.AddTextBoxColumn("SEGMENTINCOMETIME", 150)
                .SetTextAlignment(TextAlignment.Center);
            //2020.07.02 kisu.kim   인수자 추가
            // 인수자    
            grdLotList.View.AddTextBoxColumn("USERNAME", 100)
                .SetLabel("RECEIVEUSER")
                .SetTextAlignment(TextAlignment.Center);
            //LOCKING 여부
            grdLotList.View.AddTextBoxColumn("ISLOCKING")
                .SetIsHidden();
        ...
        protected override void OnToolbarSaveClick()
        {
            ...
            int check = lotList.AsEnumerable().Where(c => c.Field<string>("ISLOCKING").Equals("Y")).Count();
            string lockingLotId = null;

            if (check > 0)
            {
                foreach (DataRow row in lotList.Rows)
                {
                    if (row["ISLOCKING"].ToString() == "Y")
                    {
                        lockingLotId += row["LOTID"].ToString() + ", ";
                    }
                }

                lockingLotId = lockingLotId.Substring(0, lockingLotId.Length - 2);

                //Locking 상태인 Lot은 인수 취소를 할 수 없습니다. {0}
                throw MessageException.Create("ValidLockingLot", "\r\n Lot ID = " + string.Format(lockingLotId));
            }
            ...
            MessageWorker mw = new MessageWorker("SaveReceiveCancelLot");
        ...
        protected async override Task OnSearchAsync()
        {
            ...
            values.Add("PROCESSSTATE", "Wait");
            values.Add("PREVPROCESSSTATE", "WaitForReceive");
            values.Add("TRANSITTYPE", "ReceiveCancel");

            DataTable dtLotList = await QueryAsync("SelectLotListForProcessCancel", "10001", values);
        ...
        protected override void InitializeCondition()
        {
            ...
            CommonFunction.AddConditionAreaByAuthorityPopup("P_AREAID", 0.1, false, Conditions,false,true);
