...
{
    class BasicBounce : ArtWork.Functions.BounceInterface
    {
        public override void RD_Bounce(ArtWork.Functions.RD_Elem[] FieldA, ArtWork.Functions.RD_Elem[] FieldB, int iW, int iH, float feed, float kill, double[] distancefield, float du = 0.2097f, float dv = 0.105f)
        {
            for (int y = 0; y < iH; y++)
            {
                for (int x = 0; x < iW; x++)
        ...
        private static void SetF(ArtWork.Functions.RD_Elem[] FieldB, int x, int y, int iW, int iH, float p1, float p2)
        {
            x = (x + iW) % iW;
            y = (y + iH) % iH;
            FieldB[x + (y * iW)].R = p1;
        ...
        static ArtWork.Functions.RD_Elem GetF(ArtWork.Functions.RD_Elem[] FieldA, int x, int y, int iW, int iH)
        {
            x = (x + iW) % iW;
            y = (y + iH) % iH;
        ...
        public override void BounceN(int p, ArtWork.Functions.RD_Elem[] FieldA, ArtWork.Functions.RD_Elem[] FieldB, int iW, int iH, float feedrate, float killrate, double[] DistanceFieldBlur)
        {
            for (int i = 0; i < p / 2; i++)
            {
