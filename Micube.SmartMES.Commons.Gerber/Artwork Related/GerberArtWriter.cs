...
namespace Micube.SmartMES.Commons.Gerber
{
        ...
        public class Functions
        {
            ...
            public static void CreateArtLayersForFolder(string foldername, ArtLayerStyle Style)
            {
                ...
                foreach (string s in Directory.GetFiles(foldername))
                {
                    ...
                    {
                        Gerber.DetermineBoardSideAndLayer(s, out BoardSide Side, out BoardLayer Layer);
                ...
                if (outlinefile.Length > 0)
                {
                    ...
                    {
                        WriteArtLayerFiles(outlinefile, bottomfile, bottomsilkfile, Style);
                    }

                    if (topfile.Length > 0 && Style != ArtLayerStyle.PrototypeEdge)
                    {
                        WriteArtLayerFiles(outlinefile, topfile, topsilkfile, Style);
                    }
                }
            }
            ...
            {
                Gerber.DetermineBoardSideAndLayer(paste, out BoardSide Side, out BoardLayer Layer);
            ...
            private static void DoArtLayer(string outline, string soldermask, string target, ArtLayerStyle Style, string silk = "")
            {
                switch (Style)
                {
                    case ArtLayerStyle.PrototypeEdge:
                        ArtGeneratorProtoEdge(outline, soldermask, target, silk);
                        break;
            ...
            private static void ArtGeneratorProtoEdge(string outline, string soldermask, string target, string silk)
            {
                ...
                WriteBitmapToGerber(target + "_protoedge.gbo", Outline, Res, B, -128, false, true);
            }
            ...
            public static ParsedGerber BuildMazeFromBitmap(string filename)
            {
                ...
                while (PixelCount > 0)
                {
                    ...
                    FindPixel(M, out int sx, out int sy);
                    ...
                    if (CheckPixel(M, sx - 1, sy) == 1)
                    {
                        Checks.Add(new Point(sx - 1, sy));
                    }
                    ...
                    if (CheckPixel(M, sx, sy - 1) == 1)
                    {
                        Checks.Add(new Point(sx, sy - 1));
                    }
            ...
            {
                if (TheBounceInterface == null)
                {
                    TheBounceInterface = new BasicBounce();
                }
                ...
                for (int i = 0; i < Outline.OutlineShapes.Count; i++)
                {
                    ...
                    {
                        Outline.OutlineShapes[i].ToPolygon()
                    };
                ...
                if (true)
                {
                    for (int i = 0; i < SolderMask.OutlineShapes.Count; i++)
                    {
                        Polygons clips = new Polygons
                        {
                            SolderMask.OutlineShapes[i].ToPolygon()
                        };
                ...
                {
                    DistanceField[i] = DistMode ? 0 : 100000; AngleField[i] = 0;
                };
                                ...
                                for (int xx = Math.Max(x - Range, 0); xx < Math.Min(iW, x + Range); xx++)
                                {
                                    for (int yy = Math.Max(y - Range, 0); yy < Math.Min(iH, y + Range); yy++)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if (dist < DistanceField[xx + (yy * iW)])
                                            {
                                                DistanceField[xx + (yy * iW)] = dist;
                                            }
                                        }
                                    }
                                }
...

                {
                    for (int x = 0; x < iW; x++)
                    {
                        for (int y = 0; y < iH; y++)
                        {
                            double a = DistanceFieldBlur[x + (y * iW)];
                            //a = Math.Sqrt(a);
...

                {
                    ...
                    for (int i = 0; i < 1; i++)
                    {
                        ...
                        if (i % 100 == 0)
                        {
                            ...
                            float scaleR = 255.0f / Math.Max(0.0001f, maxR - minR);
                            float scaleG = 255.0f / Math.Max(0.0001f, maxR - minR);
                ...
                double minblurval = 100000000000;
                {
                    for (int x = 0; x < iW; x++)
                    {
                        for (int y = 0; y < iH; y++)
                        {
                            ...
                            if (newval > maxblurval)
                            {
                                maxblurval = newval;
                            }

                            if (newval < minblurval)
                            {
                                minblurval = newval;
                            }
                        }
            ...
            public class FlowerThing
            {
                ...
                private double theWidth = 0;
                private double theHeight = 0;
                ...
                public class Leaf : Item
                {
                    ...
                    public override void Draw(Graphics G)
                    {
                        ...
                        for (int i = 0; i < (int)Length; i++)
                        {
                            ...
                            V3 left = mid + (sideAnglev * Math.Sin(Math.Pow(i / Length, 1.1) * Math.PI) * Length / 5.0);
                            V3 right = mid - (sideAnglev * Math.Sin(Math.Pow(i / Length, 1.2) * Math.PI) * Length / 5.0);
                ...
                private void Branch(double x, double y, double initialdir, double count, int depth = 0)
                {
            ...
            private static void ArtGenerator_CheckerField(string outline, string soldermask, string target, string silk)
            {
                ...
                for (int i = 0; i < iW * iH; i++)
                {
                    DistanceField[i] = DistMode ? 0 : 100000;
                    AngleField[i] = 0;
                };
...

                {
                    ...
                    for (int x = 0; x < iW - 1; x++)
                    {
                        for (int y = 0; y < iH - 1; y++)
                        {
                            ...
                            V3 AA = new V3(0, 0, Math.Sqrt(a) / Range);
                            V3 BB = new V3(1, 0, Math.Sqrt(b) / Range);
                            V3 CC = new V3(0, 1, Math.Sqrt(c) / Range);
                    ...
                    for (int x = 0; x < iW; x++)
                    {
                        ...
                        else
                        {
                            if (active)
                            {
                                ...
                                //  offL *= -1;
                                //     offR *= -1;
                                pL.Add((sx / ResX) + offL, y / ResY);
                                pL.Add((x / ResX) - offR, y / ResY);
                                if (Outline != null)
                                {
    ...
    {
        private Dictionary<double, List<PolyLine>> _polyLines = new Dictionary<double, List<PolyLine>>();
        private Dictionary<double, int> _apertureLookup = new Dictionary<double, int>();
        private Dictionary<string, FontSet> _localFonts = new Dictionary<string, FontSet>();

        private List<PolyLine> _polygons = new List<PolyLine>();

        public void AddPolyLine(PolyLine a, double width = 1)
        {
            if (_polyLines.ContainsKey(width) == false)
            {
                _polyLines[width] = new List<PolyLine>();
            }

            _polyLines[width].Add(a);
        }
        ...
        public void Write(string p, double rectx = -1, double recty = -1)
        {
            ...
            foreach (var a in _polyLines.Keys)
            {
                ...
                Apts.Add(Apt);
                _apertureLookup[a] = aptindex;
                aptindex++;
            }

            foreach (var a in _polyLines)
            {
                lines.Add(Apts[_apertureLookup[a.Key]].BuildSelectGerber());
        ...
        public static PointD AttachPoint(PointD inp, PointD boxtopleft, PointD boxbottomright, ArtWork.BoxAttachment attachment)
        {
        ...
        public void DrawBoardLabelSet(Dictionary<string, FontSet> FS, ArtWork.ArtSet AS, PointD boxtopleft, PointD boxbottomright)
        {
            ...
            foreach (var T in AS.Labels)
            {
                ...
                FS.TryGetValue(T.Font.ToLower(), out FontSet F);

                if (F == null)
                {
                    _localFonts.TryGetValue(T.Font.ToLower(), out F);
                }
