...
//                                MS.Seek(0, SeekOrigin.Begin);
            ...
            foreach (var a in FileList)
            {
                ...
                else
                {
                    ...
                    {
                        Logger.AddString(string.Format("Failed to add file! {0} :{1}", a, E.ToString()));
                    }
                }
            }
        ...
        public Bitmap RenderToBitmap(int w, int h, System.Drawing.Drawing2D.Matrix Transform, Color foregroundcolor, Color backgroundcolor, ParsedGerber PLS, bool fill, float borderThick = (float)0.5,  bool forcefill = false)
        {
            Bitmap B2;
            ...
            Pen P = new Pen(foregroundcolor, borderThick);
            int Shapes = 0;
        ...
        private void AddFileToSet(MemoryStream MS, string aname, IProgressLog Logger, double drillscaler = 1.0)
        {
            ...
            Gerber.DetermineBoardSideAndLayer(aname, out BoardSide aSide, out BoardLayer aLayer);
        ...
        private int DrawLayerToGraphics(Color color, bool fill, IGraphicsInterface G2, Pen P, ParsedGerber l, bool forcefill = false)
        {
        ...
        private void DrawShape(IGraphicsInterface G, Pen P, Color c, PolyLine Shape, bool fill, double dx, double dy)
        {
            ...
            if (Points.Count() > 1)
            {
                if (fill)
                {
                    ...
                    else
                    {
                        ...
                        G.DrawLines(new Pen(c, P.Width), Points.ToArray());
                    }
                }
                else
                {
