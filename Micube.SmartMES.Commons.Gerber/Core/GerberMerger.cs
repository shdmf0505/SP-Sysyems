...
namespace Micube.SmartMES.Commons.Gerber
{
    public static class GerberMerger
    {
        ...
        public static void MultiMerge(string file1, List<string> filestomergein, string output, IProgressLog Log)
        {
            ...
            {
                DigitsBefore = Math.Max(File1Parsed.State.CoordinateFormat.DigitsBefore, MaxDigitsBefore),
                DigitsAfter = Math.Max(File1Parsed.State.CoordinateFormat.DigitsAfter, MaxDigitsAfter)
            };
...
//            Console.WriteLine("*** Writing  {0}", output);
            ...
            GerberNumberFormat GNF = new GerberNumberFormat
            {
                DigitsBefore = Math.Max(File1Parsed.State.CoordinateFormat.DigitsBefore, File2Parsed.State.CoordinateFormat.DigitsBefore),
                DigitsAfter = Math.Max(File1Parsed.State.CoordinateFormat.DigitsAfter, File2Parsed.State.CoordinateFormat.DigitsAfter)
            };
