...
namespace Micube.SmartMES.Commons.Gerber.Core.Primitives
{
    ...
    public class GerberApertureType
    {
        ...
        public override string ToString() => string.Format("D{0}: {1} ({2})", ID, Enum.GetName(typeof(GerberApertureShape), ShapeType), SourceLine);
        ...
        public bool IsLeft(PointD a, PointD b, PointD c) => (((b.X - a.X) * (c.Y - a.Y)) - ((b.Y - a.Y) * (c.X - a.X))) > 0;

        public string BuildSelectGerber() => string.Format("D{0}*", ID.ToString("D2"));
