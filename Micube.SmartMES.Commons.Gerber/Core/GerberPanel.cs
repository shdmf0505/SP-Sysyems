...
namespace Micube.SmartMES.Commons.Gerber
{
    ...
    public class GerberPanel
    {
        ...
        public void BuildAutoTabs(GerberArtWriter GAW = null, GerberArtWriter GAW2 = null)
        {
            ...
            Dictionary<Vertex, GerberInstance> InstanceMap = new Dictionary<Vertex, GerberInstance>();
            ...
            foreach (var a in R.Edges)
            {
                ...
                P.Add(A.X, A.Y);
                P.Add(B.X, B.Y);
                GerberInstance iA = null;
                GerberInstance iB = null;
        ...
        public void RenderInstanceHoles(IGraphicsInterface G, float PW, Color C, AngledThing b, bool errors = false, bool active = false, bool hover = false)
        {
            if (b == null) return;
        ...
        public void RenderInstance(IGraphicsInterface G, float PW, Color C, AngledThing b, bool errors = false, bool active = false, bool hover = false)
        {
            if (b == null) return;
            var T = G.Transform.Clone();
        ...
        private void DrawShapeNormals(IGraphicsInterface G, Pen P, PolyLine Shape)
        {
        ...
        public void DrawBoardBitmap(float PW = 1, IGraphicsInterface G = null, int targetwidth = 0, int targetheight = 0, AngledThing SelectedInstance = null, AngledThing Hoverinstance = null, double snapdistance = 1)
        {
            if (G == null)
            {
                return;
        ...
        private void FillShape(IGraphicsInterface G, SolidBrush BR, PolyLine Shape)
        {
        ...
        private static void DrawMarker(bool cross, IGraphicsInterface G, PointD pointD, float crossize, float PW, Pen P = null)
        {
            if (P == null)
        ...
        /// <param name="Shape"></param>
        private static void DrawShape(IGraphicsInterface G, Pen P, PolyLine Shape)
        {
        ...
        private void SplitPolyLineAndAddSegs(PolyLine PL)
        {
            ...
            foreach (var t in TheSet.Tabs.Where(x => x.Errors.Count == 0))
            {
                ...
                foreach (var s in Segs)
                {
                    ...
                    {
                        int ints = Helpers.FindLineCircleIntersections(center.X, center.Y, t.Radius, s.PStart, s.PEnd, out PointD I1, out PointD I2);
                        switch (ints)
                        {
                            case 0: // skip;
                                break;
                ...
                if (ints > 0)
                {
                    ...
                    if (ints == 2)
                    {
                        TabIntersection TI1 = new TabIntersection
                        {
                            Start = true,
                            Location = I1,
                            Angle = Helpers.AngleBetween(new PointD(t.Center), I1)
                        };
        ...
        public void RectanglePack()
        {
            ...
            foreach (var a in Instances.OrderByDescending(x => x.Item2))
            {
                if (GerberOutlines.ContainsKey(a.Item1.GerberPath))
                {
                    ...
                    {
                        Width = OL.TheGerber.BoundingBox.BottomRight.X - OL.TheGerber.BoundingBox.TopLeft.X,
                        Height = OL.TheGerber.BoundingBox.BottomRight.Y - OL.TheGerber.BoundingBox.TopLeft.Y
                    };
    ...
    public class GerberLayoutSet
    {
        ...
        public List<string> SaveTo(string p, Dictionary<string, GerberOutline> GerberOutlines, IProgressLog Logger)
        {
            LastExportFolder = p;
