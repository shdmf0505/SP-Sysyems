...
namespace Micube.SmartMES.Commons.Gerber
{
    ...
    public class PolyLineSet
    {
        ...
        public static ParsedGerber ParseGerber274x(List<String> inputlines, bool parseonly, bool forcezerowidth = false, GerberParserState State = null)
        {
            ...
            else
            {
                for (int i = 0; i < State.NewShapes.Count; i++)
                {
                    try
                    {
                        PolyLine PL = new PolyLine
                        {
                            ClearanceMode = State.NewShapes[i].ClearanceMode
                        };
            ...
            if (State.PreCombinePolygons)
            {
                ...
                for (int i = 0; i < Gerb.DisplayShapes.Count; i++)
                {
                    ...
                    Polygons clips = new Polygons
                    {
                        Gerb.DisplayShapes[i].ToPolygon()
                    };
        ...
        {
            //double deleteBoarderMinX = 0;
            //double deleteBoarderMinY = 0;

            //double deleteBoarderMaxX = 0;
            //double deleteBoarderMaxY = 0;
            ...
            while (State.CurrentLineIndex < lines.Count)
            {
                                                                    ...
                                                                    case 4: // outline
                                                                        {
                                                                            ...
                                                                            {
                                                                                Type = GerberApertureMacroPart.ApertureMacroTypes.Outline
                                                                            };
                                                                    ...
                                                                    default:
                                                                        {
                                                                            ...
                                                                            if (M.Length > 0)
                                                                            {
                                                                                ...
                                                                                GerberApertureMacroPart AMP = new GerberApertureMacroPart
                                                                                {
                                                                                    Type = GerberApertureMacroPart.ApertureMacroTypes.Equation,
                                                                                    EquationTarget = M.Groups["dec"].Value,
                                                                                    EquationSource = M.Groups["rightside"].Value
                                                                                };
                            ...
                            default:
                                ...
                                if (GS.Has("D") && GS.Get("D") >= 10)
                                {
                                    if (State.Apertures.TryGetValue((int)GS.Get("D"), out State.CurrentAperture) == false)
                                    {
                                        ...
                                        State.CurrentAperture = new GerberApertureType
                                        {
                                            ShapeType = GerberApertureShape.Empty
                                        };
                                    }
                                    else
                                    {
                                ...
                                else
                                {
                                    ...
                                    if (GS.Has("X") || GS.Has("Y") || GS.Has("D"))
                                    {
                                        ...
                                        if (GS.Has("X"))
                                        {
                                            ...
                                            //if (deleteBoarderMinX == X || deleteBoarderMaxX == X)
                                            //    break;
                                        }
                                        ...
                                        if (GS.Has("Y"))
                                        {
                                            ...
                                            //if (deleteBoarderMinY == Y || deleteBoarderMaxY == Y)
                                            //    break;
                                        }
                ...
                {
                    List<string> deleteLineList = lines.Where(s => s.Contains("X0") || s.Contains("Y0")).ToList();
                    foreach (string deleteLine in deleteLineList)
                    {
                        GerberSplitter GS = new GerberSplitter();
                        GS.Split(deleteLine, State.CoordinateFormat);
                        if (GS.Has("X") || GS.Has("Y") || GS.Has("D"))
                        {
                            double X = 0;
                            double Y = 0;
                            ...
                            //if (deleteBoarderMaxX <= X)
                            //    deleteBoarderMaxX = X;

                            //if (deleteBoarderMaxY <= Y)
                            //    deleteBoarderMaxY = Y;
                        }
                    }
        ...
        public class Bounds
        {
            ...
            public Bounds Grow(double v)
            {
                Bounds B = new Bounds
                {
                    TopLeft = TopLeft - new PointD(v, v),
                    BottomRight = BottomRight + new PointD(v, v)
                };
