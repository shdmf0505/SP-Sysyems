using Micube.SmartMES.Commons.Gerber.Core.Algorithms;
...
namespace Micube.SmartMES.Commons.Gerber.Core
{
    public static class Helpers
    {
            ...
            for (int i = 0; i < Paths.Count; i++)
            {
                if (Paths[i].Closed == false)
                {
                    ...
                    foreach (var a in Paths[i].Vertices)
                    {
                        ...
                        Root.CallBackInside(QueryRect, delegate (IQuadTreeItem QI)
                        {
        ...
        class SegmentEndContainer : IQuadTreeItem
        {
            public PointD Point;
        ...
        private static int FindNextMerge(List<PathDefWithClosed> Paths, out int highestnomatch, int startat = 0)
        {
            ...
            for (int i = startat; i < Paths.Count; i++)
            {
                ...
                if (P.Closed == false)
                {
                    ...
                    int startmatch = -1;
                    int endmatch = -1;
                    Root.CallBackInside(R, delegate (IQuadTreeItem QI)
                        {
                            var S = QI as SegmentEndContainer;
        ...
        public static void DrawMMGrid(IGraphicsInterface G, float PW, float width, float height, float minorGrid, float majorGrid)
        {
