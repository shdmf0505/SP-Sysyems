...
namespace Micube.SmartMES.OutsideOrderMgnt
{
    ...
    ///
    /// </summary>
    public partial class OutsourcingYoungPongCostClosing : SmartConditionManualBaseForm
    {
        #region Local Variables

        // TODO : 화면에서 사용할 내부 변수 추가

        private string _strrequsetno = "";
        private string _strPeriodid = "";
        private string _strPeriodstate = "";
        private string _strPeriodidMajor = "";
        private string _strPeriodstateMajor = "";

        #endregion Local Variables

        #region 생성자

        public OutsourcingYoungPongCostClosing()
        {
            InitializeComponent();
        }
        ...
        private void InitializeGrid_Major()
        {
            ...
            costMajorAmount.AddSpinEditColumn("OSPMAJORSETTLESUB", 100)
                .SetDisplayFormat("#,##0", MaskTypes.Numeric)
                .SetIsReadOnly();
            costMajorAmount.AddSpinEditColumn("WORKERCOUNT", 100)
                .SetDisplayFormat("#,##0", MaskTypes.Numeric);
        ...
        private void BtnSettlemantProcess_Click(object sender, EventArgs e)
        {
            ...
            if (result == System.Windows.Forms.DialogResult.Yes)
            {
                try
                {
                    ...
                    MessageWorker worker = new MessageWorker("OutsourcedCostsforMajorSuppliersSettle");
        ...
        protected async override Task OnSearchAsync()
        {
            ...
            // values.Add("P_PERIODNAME", values["P_YEARMONTH"].ToString());
            // 2021-02-23 오근영 날짜타입 (년-월-일 오전/오후 시:분:초)->(년-월) 변경
            // AND		CPR.PERIODNAME     = '2021-01-01 오전 12:00:00'
            string stryearmonth = values["P_YEARMONTH"].ToString();
            DateTime dtyearmonth = Convert.ToDateTime(stryearmonth);
            ...
            values.Add("P_PERIODTYPE", tabClose.SelectedTabPage.Name.Equals("tapExpense") ? "OutSourcing" : "MajorSuppliers");
            ...
            if (tabClose.SelectedTabPage.Name.Equals("tapExpense"))
            {
                DataTable dtSearch = await SqlExecuter.QueryAsync("GetOutsourcingYoungPongCostClosingExpense", "10001", values);
            ...
            {
                DataTable dtSearch = await SqlExecuter.QueryAsync("GetOutsourcingYoungPongCostClosingMajor", "10001", values);
                if (dtSearch.Rows.Count < 1) // 검색 조건에 해당하는 코드를 포함한 코드클래스가 없는 경우
                {
                    ShowMessage("NoSelectData"); // 조회할 데이터가 없습니다.
                }
        ...
        private void OnSaveConfrimSearch()
        {
            ...
            if (tabClose.SelectedTabPage.Name.Equals("tapExpense"))
            {
                DataTable dtSearch = SqlExecuter.Query("GetOutsourcingYoungPongCostClosingExpense", "10001", values);
                if (dtSearch.Rows.Count < 1) // 검색 조건에 해당하는 코드를 포함한 코드클래스가 없는 경우
                {
                    ShowMessage("NoSelectData"); // 조회할 데이터가 없습니다.
                }
            ...
            {
                DataTable dtSearch = SqlExecuter.Query("GetOutsourcingYoungPongCostClosingMajor", "10001", values);
                if (dtSearch.Rows.Count < 1) // 검색 조건에 해당하는 코드를 포함한 코드클래스가 없는 경우
                {
                    ShowMessage("NoSelectData"); // 조회할 데이터가 없습니다.
                }
