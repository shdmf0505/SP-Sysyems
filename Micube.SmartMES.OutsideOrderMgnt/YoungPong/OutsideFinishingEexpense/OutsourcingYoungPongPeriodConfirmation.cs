...
namespace Micube.SmartMES.OutsideOrderMgnt
{
    ...
    /// 
    /// </summary>
    public partial class OutsourcingYoungPongPeriodConfirmation : SmartConditionBaseForm
    {
        ...
        public OutsourcingYoungPongPeriodConfirmation()
        {
            InitializeComponent();
        }
        ...
        private void InitializeGrid_OspActual()
        {
            ...
            grdOspActual.View.AddTextBoxColumn("OSPMM", 120)
                .SetIsReadOnly()
                .SetTextAlignment(TextAlignment.Right)
        ...
        private void InitializationSummaryRowOspActual()
        {
            ...
            grdOspActual.View.Columns["OSPMM"].SummaryItem.SummaryType = DevExpress.Data.SummaryItemType.Sum;
            grdOspActual.View.Columns["OSPMM"].SummaryItem.DisplayFormat = "{0:###,###.####}";
        ...
        private void InitializeGrid_OspEtcAmount()
        {
            ...
            grdOspEtcAmount.View.AddTextBoxColumn("CLOSEUSERNAME", 80)
                .SetIsHidden();
            //grdOspEtcAmount.View.AddTextBoxColumn("SETTLEDATE", 120)
            //    .SetLabel("EXPSETTLEDATE")
            //    .SetDisplayFormat("yyyy-MM-dd");
            ...
            grdOspEtcAmount.View.AddComboBoxColumn("OSPETCTYPE", 100, new SqlQuery("GetCodeList", "00001", "CODECLASSID=OSPEtcType", $"LANGUAGETYPE={UserInfo.Current.LanguageType}"))
                .SetIsReadOnly();          //  작업구분           
            grdOspEtcAmount.View.AddTextBoxColumn("EXPSETTLEDEPARTMENT", 120).SetIsReadOnly();              // 정산부서
            grdOspEtcAmount.View.AddTextBoxColumn("EXPSETTLEUSER", 120).SetIsReadOnly();                 // 정산자ID
            grdOspEtcAmount.View.AddTextBoxColumn("EXPSETTLEUSERNAME", 120).SetIsReadOnly();               // 정산자명
            ...
            grdOspEtcAmount.View.AddTextBoxColumn("ACTUALDATE", 120)
                .SetDisplayFormat("yyyy-MM-dd").SetIsReadOnly();                                                                  // 작업일자

            grdOspEtcAmount.View.AddTextBoxColumn("PROCESSSEGMENTID", 80).SetIsReadOnly();        //  공정 ID
            grdOspEtcAmount.View.AddTextBoxColumn("PROCESSSEGMENTNAME", 120).SetIsReadOnly();    //  공정명
            grdOspEtcAmount.View.AddTextBoxColumn("PRODUCTDEFID", 120).SetIsReadOnly();                    // 제품 정의 ID
            grdOspEtcAmount.View.AddTextBoxColumn("PRODUCTDEFVERSION", 80).SetIsReadOnly();               // 제품 정의 Version
            grdOspEtcAmount.View.AddTextBoxColumn("PRODUCTDEFNAME", 200).SetIsReadOnly();                  // 제품명
            grdOspEtcAmount.View.AddSpinEditColumn("EXPSETTLEAMOUNT", 120)
                .SetDisplayFormat("#,##0", MaskTypes.Numeric).SetIsReadOnly();                                                   // 정산금액
            grdOspEtcAmount.View.AddTextBoxColumn("ETCDESCRIPTION", 200).SetIsReadOnly();                  //  설명
        ...
        protected async override Task OnSearchAsync()
        {
            ...
            if (tapIsExcept.SelectedTabPage.Name.Equals("tapOspActual"))
            {
                if (values["P_PROCESSSEGMENTID"].ToString().Equals("") && values["P_PRODUCTCODE"].ToString().Equals("")
                 && values["P_PRODUCTDEFNAME"].ToString().Equals("") && values["P_OSPVENDORID"].ToString().Equals("") && values["P_LOTID"].ToString().Equals(""))
                {
