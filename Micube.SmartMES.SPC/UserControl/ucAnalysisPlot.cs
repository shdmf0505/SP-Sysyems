...
namespace Micube.SmartMES.SPC.UserControl
{
    ...
    /// </summary>
    /// 
    public partial class ucAnalysisPlot : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        #region Spc Chart ShowWaitArea - Event 선언
        ...
        public void InitializeEvent()
        {
            ...
            //비교분석 BoxPlot
            this.chkBoxPlot.CheckedChanged += (s, e) =>
            {
                if (_isControlEventsStop != false) return;

                if (this.chkBoxPlot.Visible)
                {
                    ChrPoltA.Series["Series03Value"].Visible = chkBoxPlot.Checked;
                }
                else
                {
                    ChrPoltA.Series["Series03Value"].Visible = false;
                }
            };

            //산점도
            this.chkPoint.CheckedChanged += (s, e) =>
            {
                if (_isControlEventsStop != false) return;
                //((XYDiagram)ChrPoltA.Diagram).AxisY.ConstantLines["Series05OverRuleLCL"].Visible = chkPoint.Checked;
                
                if (this.chkPoint.Visible)
                {
                    ChrPoltA.Series["Series05OverRuleLCL"].Visible = chkPoint.Checked;
                }
                else
                {
                    ChrPoltA.Series["Series05OverRuleLCL"].Visible = false;
                }
                //_isCancelXBar.usl = !chkPoint.Checked;
                //ChartWholeRangeChange(_isCancelXBar, 0);
            };

            //중앙값
            this.chkMedian.CheckedChanged += (s, e) =>
            {
                if (_isControlEventsStop != false) return;
                if (this.chkMedian.Visible)
                {
                    ChrPoltA.Series["Series11AUCL"].Visible = chkMedian.Checked;
                }
                else
                {
                    ChrPoltA.Series["Series11AUCL"].Visible = false;
                }
            };

            //평균
            this.chkAverage.CheckedChanged += (s, e) =>
            {
                if (_isControlEventsStop != false) return;
                if (this.chkAverage.Visible)
                {
                    ChrPoltA.Series["Series04OverRuleUCL"].Visible = chkAverage.Checked;
                }
                else
                {
                    ChrPoltA.Series["Series04OverRuleUCL"].Visible = false;
                }
            };

            //Analysis USL
            this.chkAyUSL.CheckedChanged += (s, e) =>
            {
                if (_isControlEventsStop != false) return;
                
                if (this.chkAyUSL.Visible)
                {
                    ((XYDiagram)ChrPoltA.Diagram).AxisY.ConstantLines["ConstantLineUSL"].Visible = chkAyUSL.Checked;
                    ChrPoltA.Series["Series14OverRuleRUCL"].Visible = chkAyUSL.Checked;
                }
                else
                {
                    ((XYDiagram)ChrPoltA.Diagram).AxisY.ConstantLines["ConstantLineUSL"].Visible = false;
                    ChrPoltA.Series["Series14OverRuleRUCL"].Visible = false;
                }
                //_isCancelXBar.usl = !chkPoint.Checked;
                //ChartWholeRangeChange(_isCancelXBar, 0);
            };
            ...
            //Analysis LSL
            this.chkAyLSL.CheckedChanged += (s, e) =>
            {
                if (_isControlEventsStop != false) return;

                if (this.chkAyLSL.Visible)
                {
                    ((XYDiagram)ChrPoltA.Diagram).AxisY.ConstantLines["ConstantLineLSL"].Visible = chkAyLSL.Checked;
                    ChrPoltA.Series["Series15OverRuleRLCL"].Visible = chkAyLSL.Checked;
                }
                else
                {
                    ((XYDiagram)ChrPoltA.Diagram).AxisY.ConstantLines["ConstantLineLSL"].Visible = false;
                    ChrPoltA.Series["Series15OverRuleRLCL"].Visible = false;
                }
                //_isCancelXBar.usl = !chkPoint.Checked;
                //ChartWholeRangeChange(_isCancelXBar, 0);
            };
        ...
        /// <summary>
        /// Chart Mouse Click 이벤트 - Point별 Raw Data 표시.
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void ChrPoltA_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button != MouseButtons.Left)
            {
                return;
            ...
            if (point != null)
            {
                this.PopupRawDataPointEdit(point);
            }
        }
        /// <summary>
        /// SubGroupID 설정.
        ...
        /// <param name="isAgainAnalysisPlot"></param>
        ...
        {
            SpcClass.ChartImageCopy(this.ChrPoltA);
        }

        private void tspMnuItemImageSave_Click(object sender, EventArgs e)
        {
            SpcClass.ChartImageSave(this.ChrPoltA, cboChartType.Text.ToSafeString());
        }
        ...
        public void AnalysisExcuteCharts()
        {
            ...
            this.ChrPoltA.Series["Series02ALCL"].Visible = true; // ture여야 diagram Null 이 아님.
            ...
            this.ChrPoltA.Series["Series01AUCL"].Visible = true;

            this.ChrPoltA.Series["Series12ALCL"].Visible = false;
            this.ChrPoltA.Series["Series14OverRuleRUCL"].Visible = false;
            this.ChrPoltA.Series["Series15OverRuleRLCL"].Visible = false;

            layoutItemAyBoxPolt.Visibility = LayoutVisibility.Never;
            layoutItemAyPoint.Visibility = LayoutVisibility.Never;
            layoutItemAyAverage.Visibility = LayoutVisibility.Never;
            layoutItemAyMedian.Visibility = LayoutVisibility.Never;
            layoutItemAyUSL.Visibility = LayoutVisibility.Never;
            layoutItemAyLSL.Visibility = LayoutVisibility.Never;
            ...
            this.ChrPoltA.Series["Series13Value"].ValueDataMembers.Clear();
            this.ChrPoltA.Series["Series13Value"].Points.Clear();

            //시계열도
            this.ChrPoltA.Series["Series12ALCL"].ValueDataMembers.Clear();
            this.ChrPoltA.Series["Series12ALCL"].Points.Clear();

            this.ChrPoltA.Series["Series01AUCL"].ValueDataMembers.Clear();
            this.ChrPoltA.Series["Series01AUCL"].Points.Clear();

            this.ChrPoltA.Series["Series14OverRuleRUCL"].ValueDataMembers.Clear();
            this.ChrPoltA.Series["Series14OverRuleRUCL"].Points.Clear();

            this.ChrPoltA.Series["Series15OverRuleRLCL"].ValueDataMembers.Clear();
            this.ChrPoltA.Series["Series15OverRuleRLCL"].Points.Clear();
            ...
            diagram.SecondaryAxesX["SecondaryAxisX01"].Visibility = DevExpress.Utils.DefaultBoolean.False;
            ...
            #region Spec 조회
            double specUSL = SpcLimit.MIN;
            double specLSL = SpcLimit.MAX;

            var dataSpec = _spcPara.InputSpec.AsEnumerable();
            var partSpec = dataSpec
                .GroupBy(g => new
                {
                    g.CHARTTYPE
                })
                .Select(s => new
                {
                    nLSL = s.Where(x => x.Field<double>("LSL") != SpcLimit.MIN
                                     && x.Field<double>("LSL") != SpcLimit.MAX)
                                    .Min(x => x.Field<object>("LSL").ToSafeDoubleStaMax()),
                    nUSL = s.Where(x => x.Field<double>("USL") != SpcLimit.MIN 
                                     && x.Field<double>("USL") != SpcLimit.MAX)
                                     .Max(x => x.Field<object>("USL").ToSafeDoubleStaMin())
                });
            foreach (var item in partSpec)
            {
                specLSL = item.nLSL;
                specUSL = item.nUSL;
            }

            Console.WriteLine(string.Format("{0}{1}", specLSL, specUSL));
            ...
            try
            {
                ...
                if (specUSL != SpcLimit.MIN && specLSL != SpcLimit.MAX)
                {
                    if (specLSL < whMin)
                    {
                        whMin = specLSL;
                ...
                if (specUSL != SpcLimit.MIN && specUSL != SpcLimit.MAX)
                {
                    if (specUSL > whMax)
                    {
                        whMax = specUSL;
            ...
            switch (analysisType)
            {
                ...
                case AnalysisChartType.BoxPlot:
                    layoutItemAyBoxPolt.Visibility = LayoutVisibility.Always;
                    layoutItemAyPoint.Visibility = LayoutVisibility.Always;
                    layoutItemAyAverage.Visibility = LayoutVisibility.Always;
                    layoutItemAyMedian.Visibility = LayoutVisibility.Always;
                    layoutItemAyUSL.Visibility = LayoutVisibility.Always;
                    layoutItemAyLSL.Visibility = LayoutVisibility.Always;
                    chkBoxPlot.Checked = true;
                    chkPoint.Checked = true;
                    chkAverage.Checked = true;
                    chkMedian.Checked = true;

                    chkAyUSL.Checked = false;
                    chkAyLSL.Checked = false;


                    ChartBoxPlot(ref parPlotData, analysisType, ViewType.CandleStick, specLSL, specUSL);
                    break;
                case AnalysisChartType.AnalysisLine:
                    ...
                    layoutItemAyAverage.Visibility = LayoutVisibility.Always;
                    layoutItemAyMedian.Visibility = LayoutVisibility.Always;
                    ChartBoxPlot(ref parPlotData, analysisType, ViewType.RangeBar, specLSL, specUSL);
                    break;
                case AnalysisChartType.TimeSeries:
                    chkAyUSL.Checked = true;
                    chkAyLSL.Checked = true;
                    layoutItemAyUSL.Visibility = LayoutVisibility.Always;
                    layoutItemAyLSL.Visibility = LayoutVisibility.Always;
                    break;
                default:
            ...
            //sia작업: Analysis - 분석 Chart 표시
            //specUSL = 95; //sia작업 : 삭제할것.
            switch (analysisType)
            {
                case AnalysisChartType.AnalysisPolt:
                case AnalysisChartType.ThreePointDiagram:
                    diagram.SecondaryAxesX["SecondaryAxisX01"].Visibility = DevExpress.Utils.DefaultBoolean.False;
                    ChartThreePointDiagram(ref parPlotData, analysisType, parSubgoupID, specLSL, specUSL);
                    break;
                case AnalysisChartType.TimeSeries:
                    diagram.SecondaryAxesX["SecondaryAxisX01"].Visibility = DevExpress.Utils.DefaultBoolean.True;
                    ChartTimeSeries(ref parPlotData, analysisType, parSubgoupID, specLSL, specUSL);
                    break;
                default:
                    ((XYDiagram)ChrPoltA.Diagram).AxisY.ConstantLines["ConstantLineUSL"].Visible = false;
                    ((XYDiagram)ChrPoltA.Diagram).AxisY.ConstantLines["ConstantLineLSL"].Visible = false;
        ...
        public void ChartBoxPlot(ref SpcParameter parPlotData, AnalysisChartType chartType, ViewType chartViewType, double nLSL, double nUSL)
        {
            int rowMax = 0;
            string subgroupId = "";
            string samplingId = "";
            ...
            this.ChrPoltA.Series["Series11AUCL"].ValueDataMembers.Clear();
            this.ChrPoltA.Series["Series11AUCL"].Points.Clear();
            this.ChrPoltA.Series["Series11AUCL"].CrosshairLabelPattern = "중앙값: {V:F3}";
            ...
            if (parPlotData.SpcDataAnalysisTable != null && parPlotData.SpcDataAnalysisTable.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < rowMax; i++)
                {
                    ...
                    if (samplingName.Length < 11)
                    {
                        samplingName = string.Format("{0} Day", samplingName);
                    }
                    ...
                    this.ChrPoltA.Series["Series11AUCL"].Points.Add(seriesMedianLine);
                    ...
                    switch (chartType)
                    {
                        ...
                        case AnalysisChartType.AnalysisPolt:
                            ...
                            //seriesPointLine.DateTimeArgument = samplingName;
        ...
        public void ChartThreePointDiagram(ref SpcParameter parPlotData, AnalysisChartType chartType, string parSubgoupID, double nLSL, double nUSL)
        {
            //LCL
            this.ChrPoltA.Series["Series05OverRuleLCL"].ValueDataMembers.Clear();
            this.ChrPoltA.Series["Series05OverRuleLCL"].Points.Clear();
            ...
            diagram.AxisY.ConstantLines["ConstantLineUSL"].AxisValue = nUSL;
            diagram.AxisY.ConstantLines["ConstantLineLSL"].AxisValue = nLSL;

            string legendTextUSL = "USL";
            string legendTextLSL = "LSL";
            //USL
            if (nUSL != SpcLimit.MAX && nUSL != SpcLimit.MIN)
            {
                legendTextUSL = string.Format("{0}: {1}", legendTextUSL, nUSL.ToSafeRoundString());
                diagram.AxisY.ConstantLines["ConstantLineUSL"].Title.Text = legendTextUSL;
                diagram.AxisY.ConstantLines["ConstantLineUSL"].AxisValue = nUSL;
                diagram.AxisY.ConstantLines["ConstantLineUSL"].Visible = this.chkAyUSL.Checked;
            }
            else
            {
            ...
            //LSL
            if (nLSL != SpcLimit.MAX && nLSL != SpcLimit.MIN)
            {
                legendTextLSL = string.Format("{0}: {1}", legendTextLSL, nLSL.ToSafeRoundString());
                diagram.AxisY.ConstantLines["ConstantLineLSL"].Title.Text = legendTextLSL;
                diagram.AxisY.ConstantLines["ConstantLineLSL"].AxisValue = nLSL;
                diagram.AxisY.ConstantLines["ConstantLineLSL"].Visible = this.chkAyLSL.Checked;
            }
            else
            {
            ...
            if (parPlotData.SpcDataAnalysisTable != null && parPlotData.SpcDataAnalysisTable.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < rowMax; i++)
                {
                    ...
                    foreach (var item in partPoint)
                    {
                        ...
                        string xAsixArgument = "";
                        //seriesValuePoint.Argument = string.Format("n{0}", item.sSAMPLINGNAME);
                        if (item.sSAMPLINGNAME.Length < 11)
                        {
                            xAsixArgument = string.Format("{0} Day", item.sSAMPLINGNAME);
                        }
                        else
                        {
                            xAsixArgument = string.Format("{0}", item.sSAMPLINGNAME);
                        }

                        seriesValuePoint.Argument = xAsixArgument;

                        double[] val = new double[1];
                        val[0] = item.nNVALUE;
                        seriesValuePoint.Values = val;
                        seriesValuePoint.Color = Color.Blue;
                        this.ChrPoltA.Series["Series05OverRuleLCL"].Points.Add(seriesValuePoint);
                        //this.ChrPoltA.Series["Series13Value"].Points.Add(seriesValuePoint);


                        #region Over Rule Check
                        bool bolOverLSLCheck = false;
                        bool bolOverUSLCheck = false;
                        //LSL
                        if (nLSL != SpcLimit.MAX && nLSL != SpcLimit.MIN)
                        {
                            if (item.nNVALUE < nLSL)
                            {
                                bolOverLSLCheck = true;
                            }
                        }
                        //USL
                        if (nUSL != SpcLimit.MAX && nUSL != SpcLimit.MIN)
                        {
                            if (item.nNVALUE > nUSL)
                            {
                                bolOverUSLCheck = true;
                            }
                        }

                        //Over Rules Point USL
                        if (bolOverUSLCheck)
                        {
                            SeriesPoint seriesPointOverUSL = new SeriesPoint();
                            seriesPointOverUSL.Argument = xAsixArgument;
                            seriesPointOverUSL.Values = val;
                            seriesPointOverUSL.Color = Color.Red;
                            this.ChrPoltA.Series["Series14OverRuleRUCL"].Points.Add(seriesPointOverUSL);

                        }

                        //Over Rules Point LSL
                        if (bolOverLSLCheck)
                        {
                            SeriesPoint seriesPointOverLSL = new SeriesPoint();
                            seriesPointOverLSL.Argument = xAsixArgument;
                            seriesPointOverLSL.Values = val;
                            seriesPointOverLSL.Color = Color.Red;
                            this.ChrPoltA.Series["Series15OverRuleRLCL"].Points.Add(seriesPointOverLSL);
                        }
                        #endregion
        ...
        /// <param name="parPlotData"></param>
        /// <param name="parSubgoupID"></param>
        public void ChartTimeSeries(ref SpcParameter parPlotData, AnalysisChartType chartType, string parSubgoupID, double nLSL, double nUSL)
        {
            int nSeq = 0;
            ...
            //sia작업 : 시계열~
            var dataXGroup = _spcPara.InputData.AsEnumerable();
            var xAsix = dataXGroup
            .GroupBy(x => new { x.SAMPLING })
            .Select(s => new {
                sSAMPLING = s.Key.SAMPLING,
                sSUBGROUPNAME = s.Max(ss => ss.SAMPLINGNAME),
            });
            foreach (var item in xAsix)
            {
                SeriesPoint seriesPointSub = new SeriesPoint();
                seriesPointSub.Argument = item.sSUBGROUPNAME.ToString();
                double[] val = new double[1];
                val[0] = 0;
                seriesPointSub.Values = val;
                //seriesPoint.Color = Color.Blue;
                this.ChrPoltA.Series["Series01AUCL"].Points.Add(seriesPointSub);
            }
            ...
            foreach (var item in partPoint)
            {
                ...
                seriesPoint.Values = val;
                seriesPoint.Color = Color.Blue;
                this.ChrPoltA.Series["Series12ALCL"].Points.Add(seriesPoint);
                ...
                {
                    SeriesPoint seriesPointOverUSL = new SeriesPoint();
                    seriesPointOverUSL.Argument = nSeq.ToString();
                    seriesPointOverUSL.Values = val;
                    seriesPointOverUSL.Color = Color.Red;
                    this.ChrPoltA.Series["Series14OverRuleRUCL"].Points.Add(seriesPointOverUSL);
                ...
                {
                    SeriesPoint seriesPointOverLSL = new SeriesPoint();
                    seriesPointOverLSL.Argument = nSeq.ToString();
                    seriesPointOverLSL.Values = val;
                    seriesPointOverLSL.Color = Color.Red;
                    this.ChrPoltA.Series["Series15OverRuleRLCL"].Points.Add(seriesPointOverLSL);
                }
            ...
            this.ChrPoltA.Series["Series12ALCL"].View.Color = Color.Blue;
            this.ChrPoltA.Series["Series12ALCL"].CrosshairLabelPattern = "{A}\n측정값: {V:F3}";
            this.ChrPoltA.Series["Series12ALCL"].Visible = true;

            this.ChrPoltA.Series["Series14OverRuleRUCL"].Visible = true;
            this.ChrPoltA.Series["Series15OverRuleRLCL"].Visible = true;
        ...
        public void ControlChartDataMappingClear()
        {
            ...
            this.ChrPoltA.Series["Series14OverRuleRUCL"].ArgumentDataMember = "Label";
            this.ChrPoltA.Series["Series14OverRuleRUCL"].ValueDataMembers.AddRange(new string[] { "nROverRuleUCL" });
            this.ChrPoltA.Series["Series14OverRuleRUCL"].CrosshairLabelPattern = overRuleMessage + " USL : {V:F3}";
            this.ChrPoltA.Series["Series14OverRuleRUCL"].Visible = false;

            this.ChrPoltA.Series["Series15OverRuleRLCL"].ArgumentDataMember = "Label";
            this.ChrPoltA.Series["Series15OverRuleRLCL"].ValueDataMembers.AddRange(new string[] { "nROverRuleLCL" });
            this.ChrPoltA.Series["Series15OverRuleRLCL"].CrosshairLabelPattern = overRuleMessage + " LSL : {V:F3}";
            this.ChrPoltA.Series["Series15OverRuleRLCL"].Visible = false;
        ...
        /// </summary>
        /// <param name="point"></param>
        private void PopupRawDataPointEdit(SeriesPoint point)
        {
            ...
            //string subGroupId = string.Empty;
            //string subGroupName = string.Empty;
            double nPointValue = point.Values[0].ToSafeDoubleZero();
            //subGroupId = _SUBGROUP;//sia필수확인 : 8/8 SubGroup이 변경 되지 않음.
            //subGroupName = _SUBGROUPNAME;
            //Console.WriteLine(_controlSpec.sigmaResult.cpkResult.SUBGROUP);
            Console.WriteLine(_spcPara.InputData.Rows.Count);
            ...
            //DataTable 일경우 AsEnumerable 변환 후 처리함.
            DataTable ChartRawData = SpcDataTable.CreateDataTableChartPointRawData();
            var rowDatas = _spcPara.tableRawData.AsEnumerable();
            var query = from r in rowDatas
                        where   ((r.Field<double>("NVALUE") == nPointValue))
                        orderby (r.Field<string>("SAMPLING"))
                        orderby (r.Field<double>("NVALUE"))
                        select r;
            foreach (DataRow item in query)
            {
                ...
                SpcFunction.IsDbNckStringWrite(newRow, "SAMPLINGNAME", item, "", SpcClass.SubGroupSymbolDb, SpcClass.SubGroupSymbolView);
                newRow["LOTID"] = SpcFunction.IsDbNck(item, "LOTID");
                newRow["NVALUE"] = SpcFunction.IsDbNckDoubleMin(item, "NVALUE");
                ChartRawData.Rows.Add(newRow);
            }

            SpcStatusRawDataChartPointPopup frm = new SpcStatusRawDataChartPointPopup
            {
                RawData = ChartRawData,
                subgroupId = "subGroupId",
                gridTitle = string.Format("{0}", pointInfo)
            };
        ...
        /// <summary>
        /// Test tableRawData 테이블 구조
        /// </summary>
        /// <param name="tableName"></param>
        /// <returns></returns>
        public DataTable CreateDataTable_tableRawData(string tableName = "tableRawData")
        {
            DataTable dt = new DataTable();
            dt.TableName = tableName;
            //Filed
        ...
        public void TestDataCreate_InputData(ref SPCPara spcPara)
        {
            ...
            //string[,] pdata = _spcTestData.CTR02Group_01();//그룹 /R, S Test Data
            ...
            spcPara.tableRawData = CreateDataTable_tableRawData();
            ...
            for (i = 1; i < pdata.GetLength(1); i++)
            {
                ...
                DataRow drRaw = spcPara.tableRawData.NewRow();
                drRaw["GROUPID"] = dr["GROUPID"];
                drRaw["SAMPLEID"] = dr["SAMPLEID"];
                drRaw["SUBGROUP"] = dr["SUBGROUP"];
                drRaw["SUBGROUPNAME"] = dr["SUBGROUPNAME"];
                drRaw["SAMPLING"] = dr["SAMPLING"];
                drRaw["SAMPLINGNAME"] = dr["SAMPLINGNAME"];
                drRaw["LOTID"] = "LotId_" + i.ToString() + "_" + DateTime.Now.ToString("yyyyMMddHHmmss");
                drRaw["NVALUE"] = dr["NVALUE"];
                drRaw["NSUBVALUE"] = dr["NSUBVALUE"];
                spcPara.tableRawData.Rows.Add(drRaw);
            }
