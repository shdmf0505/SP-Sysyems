...
*/ 
    ...
    public partial class ApprovalRegistPopup : SmartPopupBaseForm, ISmartCustomPopup
    {
        ...
        /// 생성자
        /// </summary>
        public ApprovalRegistPopup(string sENTERPRISEID, string sPLANTID)
        {
            InitializeComponent();
        ...
        public ApprovalRegistPopup(string sENTERPRISEID, string sPLANTID, string sVENDORID, string sAREAID, string sPROCESSSEGMENTID,string sPROCESSSEGMENTVERSION)
        {
            InitializeComponent();
            InitializeEvent();
        ...
        private void InitializeGrid()
        {
            ...
            grdUser.View.AddTextBoxColumn("PLANTID", 100)
            ...
            grdQCApproval1.GridButtonItem = GridButtonItem.None;
            ...
            grdQCApproval2.GridButtonItem = GridButtonItem.None;
            ...
            grdQCApproval3.GridButtonItem = GridButtonItem.None;
            ...
            //grdQCApproval4.View.SetIsReadOnly();
            grdQCApproval4.GridButtonItem = GridButtonItem.None;
            grdQCApproval4.View.GridMultiSelectionMode = GridMultiSelectionMode.CheckBoxSelect;
            grdQCApproval4.View.AddTextBoxColumn("CHARGETYPE", 70).SetLabel("CLASS").SetTextAlignment(TextAlignment.Center);
            grdQCApproval4.View.AddTextBoxColumn("APPROVALSTATE", 80).SetTextAlignment(TextAlignment.Center).SetIsReadOnly();//결제상태(승인여부)
            grdQCApproval4.View.AddTextBoxColumn("USERNAME", 80).SetLabel("USERNAME").SetIsReadOnly();
            grdQCApproval4.View.AddTextBoxColumn("DEPARTMENT", 130).SetLabel("DEPARTMENT").SetIsReadOnly();
            grdQCApproval4.View.AddTextBoxColumn("EMAILADDRESS", 150).SetLabel("EMAILADDRESS").SetIsReadOnly();
            grdQCApproval4.View.AddTextBoxColumn("CHARGERID", 130).SetIsHidden();
            grdQCApproval4.View.AddTextBoxColumn("PROCESSTYPE", 130).SetIsHidden();// 절차구분
            grdQCApproval4.View.AddTextBoxColumn("SEQUENCE", 130).SetIsHidden();
            grdQCApproval4.View.AddTextBoxColumn("REJECTCOMMENTS", 130).SetIsHidden();
            grdQCApproval4.View.AddTextBoxColumn("ENTERPRISEID", 130).SetIsHidden();
            grdQCApproval4.View.AddTextBoxColumn("PLANTID", 130).SetIsHidden();
            grdQCApproval4.View.AddTextBoxColumn("APPROVALTIME", 130).SetIsHidden();
            grdQCApproval4.View.AddTextBoxColumn("DESCRIPTION", 130).SetIsHidden();
            //grdQCApproval4.View.AddTextBoxColumn("GROUP_APPROVAL", 130).SetIsHidden();
            grdQCApproval4.View.PopulateColumns();
            ...
            grdQCApproval4.View.Columns["CHARGETYPE"].ColumnEdit = repositoryItems;
            ...
            grdQCApproval4.View.Columns["APPROVALSTATE"].ColumnEdit = repositoryApprovalState;
        }
        ...
        public void InitializeEvent()
        {
            ...
            grdQCApproval4.View.ShowingEditor += grdQCApproval_ShowingEditor;
            ...
            grdQCApproval4.View.CellValueChanged += grdQCApproval4_CellValueChanged;
            ...
            grdQCApproval4.View.ShownEditor += grdApproval_ShownEditor;
            ...
            txtUserIdName.KeyDown += TxtIDNAME_KeyDown;
            //팝업저장버튼을 클릭시 이벤트
        ...
        private void grdQCApproval4_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            if (e.Column.FieldName.Equals("CHARGETYPE"))
            {
                SmartBandedGridView gridView = sender as SmartBandedGridView;
                ...
                if (dra.Length > 0)
                {
                    ...
                    grdQCApproval4.View.ClearDatas();
                    grdQCApproval4.DataSource = dra.CopyToDataTable();
                }
            }
        }
        ...
        private void BtnApplovalAdd_Click(object sender, EventArgs e)
        {
            ...
            switch (btn.Name)
            {
                ...
                case "btnAdd3":
                    ...
                    break;
                case "btnAdd4":
                    AddApprovalUser(checkedRows, grdQCApproval4, "Receiving");
                    break;
        ...
        private void BtnApplovalMinus_Click(object sender, EventArgs e)
        {
            ...
            switch (btn.Name)
            {
                ...
                case "btnMinus3":
                    ...
                    break;
                case "btnMinus4":
                    dtGrd = grdQCApproval4.DataSource as DataTable;
                    
                    checkedRows = grdQCApproval4.View.GetCheckedRows();
                    if (checkedRows == null || checkedRows.Rows.Count == 0) return;
                    foreach (DataRow dr in checkedRows.Rows)
                    {
                        if (checkedRows.AsEnumerable().Where(r => r["APPROVALSTATE"].ToString() == "Approval" || r["APPROVALSTATE"].ToString() == "Reclamation" || r["APPROVALSTATE"].ToString() == "Companion").ToList().Count > 0)
                        ...
                        (grdQCApproval4.DataSource as DataTable).AsEnumerable().Where(r => r["CHARGERID"].Equals(dr["CHARGERID"].ToString())).ToList().ForEach(row => row.Delete());
                        (grdQCApproval4.DataSource as DataTable).AcceptChanges();
                    }
                    break;
            }
        }
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            dt = grdQCApproval4.DataSource as DataTable;
            if (dt == null || dt.Rows.Count == 0)
            {
                ShowMessage("ApprovalApprobalCheck3");//수신부서가 등록되지 않았습니다.
                return;
            }
            grdQCApproval1.View.CloseEditor();
            grdQCApproval2.View.CloseEditor();
            grdQCApproval3.View.CloseEditor();
            grdQCApproval4.View.CloseEditor();
            ...
            QCApprovalDataTable.Merge(grdQCApproval4.DataSource as DataTable);
        ...
        private void ChemicalissuePopup_Load(object sender, EventArgs e)
        {
            ...
            if (QCApprovalDataTable != null && QCApprovalDataTable.Rows.Count > 0)
            {
                ...
                foreach (DataRow dr in QCApprovalDataTable.Select("PROCESSTYPE = 'Receiving'", "SEQUENCE asc"))
                {
                    (grdQCApproval4.DataSource as DataTable).ImportRow(dr);
                }
            }
            else
            {
                Dictionary<string, object> param = new Dictionary<string, object>();
                param.Add("P_USERIDNAME", txtUserIdName.EditValue);
                param.Add("P_PLANTID", cboPlant.EditValue);
        ...
        private void SetAddMinusButtonEnable()
        {
            ...
            if (lRequesterID["CHARGERID"].ToString() == sUser)//요청자가 로그인
            {
                if (lRequesterID["APPROVALSTATE"].ToString() == "Approval")//요청자가 승인
                {
                    ...
                    btnAdd4.BorderStyle = DevExpress.XtraEditors.Controls.BorderStyles.Default;
                    ...
                    btnMinus4.BorderStyle = DevExpress.XtraEditors.Controls.BorderStyles.Default;
                ...
                else//요청자가 승인 안함
                {
                    ...
                    btnAdd4.BorderStyle = DevExpress.XtraEditors.Controls.BorderStyles.Style3D;
                    ...
                    btnMinus4.BorderStyle = DevExpress.XtraEditors.Controls.BorderStyles.Style3D;
                }
            }
            else// 결재자 로그인
            {
                ...
                if ((IReceiving.Where(r => r["APPROVALSTATE"].ToString() == "Approval").Count() == IReceiving.Count() && IReceiving.Count() > 0)//모두 승인됨
                    || IReceiving.Where(r => r["CHARGERID"].ToString() == sUser).Count() == 0//결재부서에 없음
                    || IReceiving.Where(r => r["CHARGERID"].ToString() == sUser && r["APPROVALSTATE"].ToString() == "Approval").Count() == 1//승인자
                    || !CheckSequence(IReceiving))//결재순서
                {
                    //Receiving 비활성화
                    btnAdd4.Enabled = false;
                    btnMinus4.Enabled = false;
        ...
        private void AddApprovalUser(DataTable dt, SmartBandedGrid grd, string sPROCESSTYPE)
        {
            ...
            DataTable dt4 = grdQCApproval4.DataSource as DataTable;
        ...
        private void InitializeComboBox()
        {
            ...
            cboPlant.UseEmptyItem = true;
            cboPlant.EditValue = UserInfo.Current.Plant;
            cboPlant.DataSource = SqlExecuter.Query("GetPlantListByQcm", "10001");
