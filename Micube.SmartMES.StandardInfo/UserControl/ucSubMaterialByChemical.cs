...
namespace Micube.SmartMES.StandardInfo
{
    ...
    /// 업  무  설  명  : 공정이 약품인 경우에 발생되는 user control
    /// 생    성    자  : 전우성
    /// 생    성    일  : 2020-12-28
    /// 필  수  처  리  :
    ///
    /// </summary>
    public partial class ucSubMaterialByChemical : ISubMaterialBomRouting
    {
        ...
        /// 생성자
        /// </summary>
        public ucSubMaterialByChemical(Dictionary<string, object> parentDic)
        {
            _dic = parentDic;
        ...
        private void InitializeGrid()
        {
            ...
            grdMain.View.AddSpinEditColumn("BOMQTYSTANDARD", 120).SetDisplayFormat("###,###,##0.#########", MaskTypes.Numeric).SetIsReadOnly().SetTextAlignment(TextAlignment.Right);
            grdMain.View.AddSpinEditColumn("PNLBYAMOUNTBOM", 120).SetDisplayFormat("###,###,##0.#########", MaskTypes.Numeric).SetIsReadOnly().SetTextAlignment(TextAlignment.Right);
            grdMain.View.AddSpinEditColumn("PNLAREA", 120).SetLabel("PNLAREAM2").SetDisplayFormat("###,###,##0.#########", MaskTypes.Numeric).SetIsReadOnly().SetTextAlignment(TextAlignment.Right);
        ...
        public void InitializeEvent()
        {
            ...
            {
                if (!e.Column.FieldName.Equals("WORKPLANE"))
                {
                    return;
                }

                double pnlArea = e.Value.Equals("Double") ? (Format.GetDouble(_dic["PNLX"], 0) * Format.GetDouble(_dic["PNLY"], 0)) / 1000000
                                                          : ((Format.GetDouble(_dic["PNLX"], 0) * Format.GetDouble(_dic["PNLY"], 0)) / 1000000) / 2;

                grdMain.View.GetDataRow(e.RowHandle)["PNLAREA"] = pnlArea;
                grdMain.View.GetDataRow(e.RowHandle)["PNLBYAMOUNTBOM"] = pnlArea * Format.GetDouble(grdMain.View.GetDataRow(e.RowHandle)["BOMQTYSTANDARD"], 0);
            };
        }
        ...
        public override async Task Search()
        {
            try
            {
                ...
                if (await SqlExecuter.QueryAsync("SelectSubMaterialDivisionByChemicalBOMRouting", "10001", _dic) is DataTable dt)
                {
        ...
        public override void SetDateAsync()
        {
            try
            {
                ...
                if (SqlExecuter.Query("SelectSubMaterialDivisionByChemicalBOM", "10001", _dic) is DataTable dt)
                {
