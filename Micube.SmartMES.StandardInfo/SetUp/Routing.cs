...
namespace Micube.SmartMES.StandardInfo
{
    ...
    /// 수정 이 력 : 
    /// </summary> 
    public partial class Routing : SmartConditionBaseForm
	{
		...
		private void InitializeGridList()
		{
            ...
            //TAB 0 : 포준공정 그리드 초기화
            ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            //grdProcessSegmentClass.View.GridMultiSelectionMode = GridMultiSelectionMode.CheckBoxSelect;
            ...
            //         grdProcessSegmentClass.View.AddComboBoxColumn("PROCESSSEGMENTCLASSTYPE", 100, new SqlQuery("GetCodeList", "00001", "CODECLASSID=ProcessSegmentClassType", $"LANGUAGETYPE={UserInfo.Current.LanguageType}"))
            //         .SetIsReadOnly();
            //         //grdProcessSegmentClass.View.SetKeyColumn("PROCESSSEGMENTCLASSID");
            //         grdProcessSegmentClass.View.AddTextBoxColumn("PROCESSSEGMENTCLASSID", 150).SetValidationIsRequired();
            //grdProcessSegmentClass.View.AddTextBoxColumn("PROCESSSEGMENTCLASSNAME", 150);
            //         grdProcessSegmentClass.View.AddTextBoxColumn("PARENTPROCESSSEGMENTCLASSID",150).SetIsReadOnly();
            //         grdProcessSegmentClass.View.AddTextBoxColumn("PARENTPROCESSSEGMENTCLASSNAME", 150).SetIsReadOnly();
            ...
            // 공정 최소 최대 
            grdOpeMinMax.View.GridMultiSelectionMode = GridMultiSelectionMode.CheckBoxSelect;
            
            grdOpeMinMax.View.AddTextBoxColumn("PLANTID").SetIsHidden();
            grdOpeMinMax.View.AddTextBoxColumn("OPERATIONID").SetIsHidden();
            
            grdOpeMinMax.View.AddTextBoxColumn("PLANTNAME", 80);
            grdOpeMinMax.View.AddTextBoxColumn("OPERATIONIDMIN",80);
            grdOpeMinMax.View.AddTextBoxColumn("OPERATIONIDMAX", 80);
            grdOpeMinMax.View.PopulateColumns();

            // 공정
            grdOperation.GridButtonItem = GridButtonItem.All;
            grdOperation.View.AddTextBoxColumn("PLANTID");
            ...
            if (dataRow["OPERATIONID"].ToString() != "")
            {
                ...
                switch (focusRow["MASTERDATACLASSID"].ToString())
                {
                    ...
                    case "":
                        ...
                        break;
                    default:
                        ParamComp.Add("ASSEMBLYITEMID", focusRow["PARENTS_ASSEMBLYITEMID"]);
                        ParamComp.Add("ASSEMBLYITEMVERSION", focusRow["PARENTS_ASSEMBLYITEMVERSION"]);
                        break;
                }
               // ParamComp.Add("ASSEMBLYITEMID", dataRow["ASSEMBLYITEMID"].ToString());
               // ParamComp.Add("ASSEMBLYITEMVERSION", dataRow["ASSEMBLYITEMVERSION"].ToString());
                ...
                ParamValue.Add("ITEMID", dataRow["ASSEMBLYITEMID"].ToString());
                ParamValue.Add("ITEMVERSION", dataRow["ASSEMBLYITEMVERSION"].ToString());
        ...
        private void grdOperation_AddingNewRow(Framework.SmartControls.Grid.BandedGrid.SmartBandedGridView sender, Framework.SmartControls.Grid.AddNewRowArgs args)
        {
            ...
            DataRow dataRow = treeRouting.GetFocusedDataRow();
            ...
            if (dataRow["MASTERDATACLASSID"].ToString() == "OperationItem")
            {
                sASSEMBLYITEM = dataRow["PARENTS_ASSEMBLYITEMID"].ToString() + dataRow["PARENTS_ASSEMBLYITEMVERSION"].ToString();
        ...
        private void TreeRouting_FocusedNodeChanged(object sender, DevExpress.XtraTreeList.FocusedNodeChangedEventArgs e)
        {
            ...
            if (focusRow != null)
            {
                ...
                //WarehouseCode.Id = "WAREHOUSEID";
                //WarehouseCode.LabelText = "WAREHOUSEID";
                //WarehouseCode.SearchQuery = new SqlQuery("GetWarehouseList", "10001", $"ENTERPRISEID={UserInfo.Current.Enterprise}", $"PLANTID={focusRow["PLANTID"].ToString()}");
                //WarehouseCode.IsMultiGrid = false;
                //WarehouseCode.DisplayFieldName = "COMPLETEWAREHOUSENAME";
                //WarehouseCode.ValueFieldName = "COMPLETEWAREHOUSEID";
                //WarehouseCode.LanguageKey = "WAREHOUSEID";
                ...
                //Dictionary<string, object> Param = new Dictionary<string, object>();
                //Param.Add("ITEMID", focusRow["ASSEMBLYITEMID"].ToString());
                //Param.Add("ITEMVERSION", focusRow["ASSEMBLYITEMVERSION"].ToString());
                //Param.Add("ITEMNAME", "");
                //Param.Add("SEARCH", "");
                ...
                ////공장
                //cboSite1.EditValue = focusRow["PLANTID"].ToString();
                ////단위
                ...
                //DataTable dRouting = SqlExecuter.Query("GetRouting", "10001", values);


                Dictionary<string, object> ParamRouting = new Dictionary<string, object>();

                ParamRouting.Add("ENTERPRISEID", UserInfo.Current.Enterprise);
                ParamRouting.Add("ITEMID", focusRow["ASSEMBLYITEMID"].ToString());
                ParamRouting.Add("ITEMVERSION", focusRow["ASSEMBLYITEMVERSION"].ToString());
                DataTable dRouting = SqlExecuter.Query("GetRouting", "10001", ParamRouting);
                grdRouting.DataSource = dRouting;
                SetControlsFrom scf = new SetControlsFrom();
                scf.SetControlsFromTable(smartSplitTableLayoutPanel4, dRouting);
                ...
                if (focusRow["MASTERDATACLASSID"].ToString() != "Product")
                {
                    sPlantid = focusRow["PLANTID"].ToString();
                }

                string sMASTERDATACLASSID = "";
                if (focusRow["MASTERDATACLASSID"].ToString() != "SubAssembly")
                {
                    sMASTERDATACLASSID = "OperationItem";
                }
                else
                {
                    sMASTERDATACLASSID = "SubAssembly";
                }

                param.Add("PLANTID", sPlantid);
                param.Add("LANGUAGETYPE", UserInfo.Current.LanguageType);
                param.Add("MASTERDATACLASSID", sMASTERDATACLASSID);

                param.Add("PARENTS_ASSEMBLYITEMID", focusRow["PARENTS_ASSEMBLYITEMID"].ToString());
                param.Add("PARENTS_ASSEMBLYITEMVERSION", focusRow["PARENTS_ASSEMBLYITEMVERSION"].ToString());

                var values = Conditions.GetValues();
                param.Add("VALIDSTATE", values["ROUTINGVALIDSTATE"]);
                
                grdOperation.DataSource = SqlExecuter.Query("GetOperationlist", "10001", param);
        ...
        private void BtnItemCreate_Click(object sender, EventArgs e)
        {
            ...
            DataTable dtBom1 = new DataTable();
            ...
            if(rowFocu["MASTERDATACLASSID"].ToString() == "SubAssembly")
            {
                ShowMessage("NoSubAssemblyPlantRouting");
                return;
            }

            // 공정 테이블 생성
            DataTable dtOperation1 = new DataTable();
            dtOperation1.Columns.Add("OPERATIONID");
            dtOperation1.Columns.Add("OPERATIONSEQUENCE");
            dtOperation1.Columns.Add("USERSEQUENCE");
            dtOperation1.Columns.Add("ENTERPRISEID");
            dtOperation1.Columns.Add("PLANTID");
            dtOperation1.Columns.Add("ASSEMBLYROUTINGID");
            dtOperation1.Columns.Add("VALIDSTATE");
            dtOperation1.Columns.Add("PROCESSUOM");
            dtOperation1.Columns.Add("_STATE_");
            ...
            if (dtRouting.Rows.Count != 0)
            {
                ...
                rowPROCESSDEFINITION["PROCESSCLASSID"] = "Product";
                ...
                // 제품공정 데이터 생성
                DataRow rowOperation = dtOperation1.NewRow();

                //rowOperation["OPERATIONID"] = number.GetStdNumber("Operation", "OPE" + sdate);


                string sOPERATIONID =  dtRouting.Rows[0]["ASSEMBLYITEMID"].ToString() + dtRouting.Rows[0]["ASSEMBLYITEMVERSION"].ToString();

                rowOperation["OPERATIONID"] = number.GetStdNumber("Operation", sOPERATIONID);
                ...
                rowOperation["_STATE_"] = "added";
                rowOperation["VALIDSTATE"] = "Valid";
                
                if (UserInfo.Current.Enterprise == "YOUNGPOONG")
                {
                    rowOperation["PROCESSUOM"] = "PNL";
                }

                dtOperation1.Rows.Add(rowOperation);

                //BOM 테이블 생성
                //DataTable dtBom1 = new DataTable();
                dtBom1.Columns.Add("ASSEMBLYBOMID", typeof(Int32));
                dtBom1.Columns.Add("ENTERPRISEID");
                dtBom1.Columns.Add("PLANTID");

                dtBom1.Columns.Add("ASSEMBLYITEMID");
                dtBom1.Columns.Add("ASSEMBLYITEMVERSION");
                dtBom1.Columns.Add("ASSEMBLYITEMUOM");
                dtBom1.Columns.Add("ASSEMBLYITEMCLASS");

                dtBom1.Columns.Add("ASSEMBLYTYPE");
                dtBom1.Columns.Add("_STATE_");
                dtBom1.Columns.Add("VALIDSTATE");
                
                // 채번 시리얼 존재 유무 체크


                //BOM 데이터 생성
                DataRow rowBom = dtBom1.NewRow();
                ...
                dtBom1.Rows.Add(rowBom);

                //dtItemserialI.TableName = "idclassserial";
                dtRouting.TableName = "routing";
                dtOperation1.TableName = "operation";
                dtBom1.TableName = "assemblybillofmaterial";
                dtPROCESSDEFINITION.TableName = "processdefinition";

                DataSet dsChangTot = new DataSet();
                //dsChangTot.Tables.Add(dtItemserialI);
                dsChangTot.Tables.Add(dtRouting);
                dsChangTot.Tables.Add(dtOperation1);
                dsChangTot.Tables.Add(dtBom1);
                dsChangTot.Tables.Add(dtPROCESSDEFINITION);

                ExecuteRule("Routing", dsChangTot);

            }
            else
            {
                dtRouting = (DataTable)grdRouting.DataSource;
                dtRouting.Columns.Add("_STATE_");
            }

            //object obj = grdRouting.DataSource;
            //DataTable dtRouting = (DataTable)obj;
            DataTable dtRoutingI = dtRouting.Clone();
            //dtRoutingI.Columns.Add("_STATE_");
            ...
            dtItemMaster = dt.Clone();


            DataTable dtChk = grdOpeMinMax.View.GetCheckedRows();
            ...
            {
                DataRow rowIm = dtItemMaster.NewRow();
                ...
                foreach (DataRow rowItem in dt.Rows)
                {
                    foreach(DataColumn col in dt.Columns)
                    {
                        switch(col.ColumnName)
                        {
                            ...
                            case "MASTERDATACLASSID":
                                rowIm[col.ColumnName] = "OperationItem";
                                break;
                            case "ITEMID":

                                // 인터
                                string sItemcode = "";
                                if (row["PLANTID"].ToString() == "IFC")
                                {
                                    sItemcode = "4F";
                                }
                                if (row["PLANTID"].ToString() == "CCT")
                                {
                                    sItemcode = "4C";
                                }
                                if (row["PLANTID"].ToString() == "IFV")
                                {
                                    sItemcode = "4V";
                                }
                                ...
                                // 영풍
                                if (row["PLANTID"].ToString() == "YPE")
                                {
                                    sItemcode = "4H";
                                }
                                if (row["PLANTID"].ToString() == "YPEV")
                                {
                                    sItemcode = "4V";
                                }

                                switch (row["PLANTID"].ToString())
                                {
                                    case "IFC":
                                    case "CCT":
                                    case "IFV":
                                        // 인터사이트
                                        rowIm[col.ColumnName] = sItemcode + dt.Rows[0]["ITEMID"].ToString().Substring(2);
                                        break;
                                    case "YPE":
                                    case "YPEV":
                                        // 영품사이트
                                        rowIm[col.ColumnName] = sItemcode + dt.Rows[0]["ITEMID"].ToString();
                                        break;
                                }

                                break;
                            case "PLANTID":
                                rowIm[col.ColumnName] = row["PLANTID"].ToString();
                                break;
                            case "ITEMVERSION":
                                rowIm[col.ColumnName] = dt.Rows[0]["ITEMVERSION"].ToString();

                                break;
                            case "_STATE_":
                                rowIm[col.ColumnName] = "added";
                                break;
                            case "VALIDSTATE":
                                rowIm[col.ColumnName] = "Valid";
                                break;
                            default:
                                rowIm[col.ColumnName] = rowItem[col.ColumnName];
                                break;
                        }
                    }
                ...
                dtItemMaster.Rows.Add(rowIm);
            }


            foreach (DataRow rowImC in dtItemMaster.Rows)
            {

                DataRow drRoutingI = dtRoutingI.NewRow();

                foreach (DataColumn colRouting in dtRoutingI.Columns)
                {
                    switch (colRouting.ColumnName)
                    {
                        case "ASSEMBLYITEMID":
                            drRoutingI[colRouting.ColumnName] = rowImC["ITEMID"];
                            break;
                        case "ASSEMBLYITEMVERSION":
                            drRoutingI[colRouting.ColumnName] = rowImC["ITEMVERSION"];
                            break;
                        case "ASSEMBLYITEMCLASS":
                            drRoutingI[colRouting.ColumnName] = "OperationItem";
                            break;
                        case "ASSEMBLYROUTINGID":
                            drRoutingI[colRouting.ColumnName] = number.GetStdNumber("Routing", "ROT" + sdate);  
                            break;
                        case "PLANTID":
                            drRoutingI[colRouting.ColumnName] = rowImC["PLANTID"].ToString();
                            break;
                        case "_STATE_":
                            drRoutingI[colRouting.ColumnName] = "added";
                            break;
                        case "VALIDSTATE":
                            drRoutingI[colRouting.ColumnName] = "Valid";
                            break;
                        default:
                            drRoutingI[colRouting.ColumnName] = dtRouting.Rows[0][colRouting.ColumnName];
                            break;
                    }
                }
                dtRoutingI.Rows.Add(drRoutingI);
            }
            ...
            dtOperation.Columns.Add("USERSEQUENCE", typeof(decimal));
            ...
            dtOperation.Columns.Add("PROCESSUOM");

            foreach (DataRow row in grdOpeMinMax.View.GetCheckedRows().Rows)
            {
                for(int rowI = int.Parse(row["OPERATIONIDMIN"].ToString()); rowI <= int.Parse(row["OPERATIONIDMAX"].ToString()); rowI = rowI + 10)
                {
                    // 공정 데이터 생성

                    //DataRow[] rowOperMinMax = dtOperMinMax.Select("OPERATIONID <> ''");
                    DataRow rowOperation = dtOperation.NewRow();

                 

                    string sASSEMBLYITEM = dtRouting.Rows[0]["ASSEMBLYITEMID"].ToString() + dtRouting.Rows[0]["ASSEMBLYITEMVERSION"].ToString();

                    string sOPERATIONID = number.GetStdNumber("Operation", sASSEMBLYITEM);
                    rowOperation["OPERATIONID"] = sOPERATIONID;
                    rowOperation["OPERATIONSEQUENCE"] = rowI;
                    rowOperation["USERSEQUENCE"] = rowI;
                    rowOperation["ENTERPRISEID"] = dtRoutingI.Rows[0]["ENTERPRISEID"].ToString();
                    rowOperation["PLANTID"] = row["PLANTID"].ToString();
                    //공정반제품 번호 
                    DataRow[] rowRoutingI = dtRoutingI.Select("PLANTID = '" + row["PLANTID"].ToString() + "'");
                    rowOperation["ASSEMBLYROUTINGID"] = rowRoutingI[0]["ASSEMBLYROUTINGID"].ToString();
                    rowOperation["_STATE_"] = "added";
                    rowOperation["VALIDSTATE"] = "Valid";
                    ...
                    rowPROCESSPATH["PROCESSPATHID"] = sOPERATIONID;
                    rowPROCESSPATH["ENTERPRISEID"] = dtRoutingI.Rows[0]["ENTERPRISEID"].ToString();
                    rowPROCESSPATH["PLANTID"] = row["PLANTID"].ToString();
                    rowPROCESSPATH["PROCESSDEFID"] = rowFocu["ASSEMBLYITEMID"].ToString();
                    rowPROCESSPATH["PROCESSDEFVERSION"] = rowFocu["ASSEMBLYITEMVERSION"].ToString();
                    rowPROCESSPATH["PROCESSSEGMENTID"] = "";
                    rowPROCESSPATH["PROCESSSEGMENTVERSION"] = "";
                    rowPROCESSPATH["PATHSEQUENCE"] = rowI;
                    rowPROCESSPATH["USERSEQUENCE"] = rowI;
                    rowPROCESSPATH["PATHTYPE"] = "Normal";
                    rowPROCESSPATH["VALIDSTATE"] = "Valid";
                    rowPROCESSPATH["_STATE_"] = "added";

                    if (UserInfo.Current.Enterprise == "YOUNGPOONG")
                    {
                        rowPROCESSPATH["PROCESSUOM"] = "PNL";
                    }
            ...
            dtBom.Columns.Add("ASSEMBLYBOMID");
            ...
            dtComp.Columns.Add("ASSEMBLYBOMID");
            dtComp.Columns.Add("ENTERPRISEID");
            dtComp.Columns.Add("PLANTID");
            dtComp.Columns.Add("OPERATIONID");
            dtComp.Columns.Add("OPERATIONSEQUENCE");
            dtComp.Columns.Add("COMPONENTBOMID");
            dtComp.Columns.Add("COMPONENTSEQUENCE");

            dtComp.Columns.Add("COMPONENTITEMID");
            dtComp.Columns.Add("COMPONENTITEMVERSION");
            dtComp.Columns.Add("_STATE_");
            dtComp.Columns.Add("VALIDSTATE");
            

            //최소 오퍼
            DataTable dtMin = new DataTable();
            dtMin.Columns.Add("COMPONENTITEMID");
            dtMin.Columns.Add("COMPONENTITEMVERSION");
            dtMin.Columns.Add("USERSEQUENCE", typeof(decimal));
            dtMin.Columns.Add("PLANTID");
            
            dtMin.Columns.Add("OPERATIONID");
            dtMin.Columns.Add("OPERATIONSEQUENCE");

            foreach (DataRow rowRoutingMin in dtRoutingI.Rows)
            {
                DataRow[] rowRouting = dtOperation.Select("ASSEMBLYROUTINGID = '" + rowRoutingMin["ASSEMBLYROUTINGID"].ToString() + "'", "USERSEQUENCE ASC");

                DataRow rowMinAdd = dtMin.NewRow();
                rowMinAdd["COMPONENTITEMID"] = rowRoutingMin["ASSEMBLYITEMID"].ToString();
                rowMinAdd["COMPONENTITEMVERSION"] = rowRoutingMin["ASSEMBLYITEMVERSION"].ToString();
                rowMinAdd["USERSEQUENCE"] = rowRouting[0]["USERSEQUENCE"].ToString();

                rowMinAdd["OPERATIONID"] = rowRouting[0]["OPERATIONID"].ToString();
                rowMinAdd["OPERATIONSEQUENCE"] = rowRouting[0]["OPERATIONSEQUENCE"].ToString();
                rowMinAdd["PLANTID"] = rowRouting[0]["PLANTID"].ToString();

                dtMin.Rows.Add(rowMinAdd);

            }

            foreach (DataRow rowRoutingI in dtRoutingI.Rows)
            {
                //BOM 데이터 생성
                DataRow rowBom = dtBom.NewRow();
                rowBom["ASSEMBLYBOMID"] = number.GetStdNumber("AssemblyBomId", "");
                rowBom["ENTERPRISEID"] = rowRoutingI["ENTERPRISEID"].ToString();
                rowBom["PLANTID"] = rowRoutingI["PLANTID"].ToString();
                rowBom["ASSEMBLYITEMID"] = rowRoutingI["ASSEMBLYITEMID"].ToString();
                rowBom["ASSEMBLYITEMVERSION"] = rowRoutingI["ASSEMBLYITEMVERSION"].ToString();
                rowBom["ASSEMBLYITEMUOM"] = cboUom.GetDataValue();
                rowBom["ASSEMBLYITEMCLASS"] = rowRoutingI["ASSEMBLYITEMCLASS"].ToString();
                rowBom["ASSEMBLYTYPE"] = "Phantom"; // 투입유형
                rowBom["_STATE_"] = "added";
                rowBom["VALIDSTATE"] = "Valid";
                dtBom.Rows.Add(rowBom);
                

                if (dtMin.Rows.Count != 1)
                {

                    DataRow[] drowUSERSEQUENCE = dtMin.Select("COMPONENTITEMID = '" + rowRoutingI["ASSEMBLYITEMID"].ToString() + "'");


                    decimal dUSERSEQUENCE = decimal.Parse(drowUSERSEQUENCE[0]["USERSEQUENCE"].ToString());

                    DataRow[] rowMin = dtMin.Select("USERSEQUENCE < " + dUSERSEQUENCE + " ", "USERSEQUENCE DESC");

                    if (rowMin.Length != 0)
                    {
                      

                        DataRow rowComp = dtComp.NewRow();
                        rowComp["ASSEMBLYBOMID"] = dtBom.Rows[0]["ASSEMBLYBOMID"].ToString();
                        rowComp["ENTERPRISEID"] = rowRoutingI["ENTERPRISEID"].ToString();
                        //rowComp["PLANTID"] = rowRoutingI["PLANTID"].ToString();
                        rowComp["PLANTID"] = rowMin[0]["PLANTID"].ToString();
                        rowComp["OPERATIONID"] = drowUSERSEQUENCE[0]["OPERATIONID"].ToString();
                        rowComp["OPERATIONSEQUENCE"] = drowUSERSEQUENCE[0]["OPERATIONSEQUENCE"].ToString();
                        rowComp["COMPONENTBOMID"] = number.GetStdNumber("BomComponent", "");
                        rowComp["COMPONENTSEQUENCE"] = drowUSERSEQUENCE[0]["OPERATIONSEQUENCE"].ToString();
                        rowComp["COMPONENTITEMID"] = rowMin[0]["COMPONENTITEMID"].ToString();
                        rowComp["COMPONENTITEMVERSION"] = rowMin[0]["COMPONENTITEMVERSION"].ToString();

                        rowComp["_STATE_"] = "added";
                        rowComp["VALIDSTATE"] = "Valid";
            ...
            DataRow rowgrdRouting = grdRouting.View.GetFocusedDataRow();

            DataRow[] rowMinP = dtMin.Select("1=1", "USERSEQUENCE DESC");

            DataRow rowCompP = dtComp.NewRow();
            if( dtBom1 != null)
            {
                if(dtBom1.Rows.Count !=0)
                {
                    rowCompP["ASSEMBLYBOMID"] = dtBom1.Rows[0]["ASSEMBLYBOMID"];
                }
                else
                {
                    rowCompP["ASSEMBLYBOMID"] = rowgrdRouting["ASSEMBLYBOMID"];
                }
            ...
            rowCompP["ENTERPRISEID"] = rowgrdRouting["ENTERPRISEID"];
            rowCompP["PLANTID"] = rowMinP[0]["PLANTID"];

            if (dtOperation1 != null)
            {
                if (dtOperation1.Rows.Count != 0)
                {
                    rowCompP["OPERATIONID"] = dtOperation1.Rows[0]["OPERATIONID"];
                }
                else
                {
                    rowCompP["OPERATIONID"] = rowgrdRouting["OPERATIONID_PAR"];
                }
            ...
            rowCompP["OPERATIONSEQUENCE"] = 1;
            rowCompP["COMPONENTBOMID"] = number.GetStdNumber("BomComponent", "");
            rowCompP["COMPONENTSEQUENCE"] = 1;

            rowCompP["COMPONENTITEMID"] = rowMinP[0]["COMPONENTITEMID"].ToString();
            rowCompP["COMPONENTITEMVERSION"] = rowMinP[0]["COMPONENTITEMVERSION"].ToString();
            rowCompP["_STATE_"] = "added";
            rowCompP["VALIDSTATE"] = "Valid";
            dtComp.Rows.Add(rowCompP);

            dtItemMaster.TableName = "itemmaster";
            dtRoutingI.TableName = "routing";
            dtOperation.TableName = "operation";
            dtBom.TableName = "assemblybillofmaterial";
            dtComp.TableName = "bomcomponent";

            dtPROCESSPATH.TableName = "processPath";
            //dtPROCESSDEFINITION.TableName = "processdefinition";

           
            // 공정반제품이 품목마스터에 존재 할경우 추가 하지 않음
            //foreach(DataRow rowIm in  dtItemMaster.Rows)
            //{
            //    Dictionary<string, object> ParamIm = new Dictionary<string, object>();
            //    ParamIm.Add("ENTERPRISEID", UserInfo.Current.Enterprise);
            //    ParamIm.Add("ITEMID", rowIm["ITEMID"]);
            //    ParamIm.Add("ITEMVERSION", rowIm["ITEMVERSION"]);
            //    DataTable dtIm = SqlExecuter.Query("GetItemidDuplicate", "10001", ParamIm);
            //    if(dtIm != null)
            //    {
            ...
            DataSet dsChangInsert = new DataSet();
            dsChangInsert.Tables.Add(dtItemMaster);
            dsChangInsert.Tables.Add(dtRoutingI);
            dsChangInsert.Tables.Add(dtOperation);
            dsChangInsert.Tables.Add(dtBom);
            dsChangInsert.Tables.Add(dtComp);

            dsChangInsert.Tables.Add(dtPROCESSPATH);
            //dsChangInsert.Tables.Add(dtPROCESSDEFINITION);

            ExecuteRule("Routing", dsChangInsert);
            ...
            ParamStartend.Add("ENTERPRISEID", rowFocu["ENTERPRISEID"]);
            ParamStartend.Add("PROCESSDEFID", rowFocu["ASSEMBLYITEMID"]);
            ParamStartend.Add("PROCESSDEFVERSION", rowFocu["ASSEMBLYITEMVERSION"]);
            

            DataTable dtminmax = SqlExecuter.Query("GetRoutingProcesspathList", "10001", ParamStartend);

            DataTable dtminmaxI = dtminmax.Copy();
        ...
        private void InitializeTree()
        {
           ...
           // treeRouting.SetEmptyRoot(rowFocu["ASSEMBLYITEMID"].ToString() + " " + rowFocu["ASSEMBLYITEMVERSION"].ToString(), "A");
            ...
            DataTable dt = SqlExecuter.Query("GetRountingTree", "10001", param);
        ...
        /// OperationMinmax 초기화
        /// </summary>
        private void InitializeOperationMinmax()
        {
            ...
            DataTable dt =  SqlExecuter.Query("GetOperationMinmax", "10001", param);

          

            foreach (DataRow row in dt.Rows)
            {
                if(dttreeSet.Rows.Count != 0)
                {
                    DataRow[] drtreeSET = dttreeSet.Select("USERSEQUENCE <> '1' AND  PLANTID = '" + row["PLANTID"].ToString() + "' AND FG_OPERATION = 'Y' AND MASTERDATACLASSID = 'OperationItem'", "USERSEQUENCE ASC");

                    if(drtreeSET.Length !=0)
                    {
                        row["OPERATIONIDMIN"] = dttreeSet.Compute("MIN(USERSEQUENCE)", "PLANTID = '" + row["PLANTID"].ToString() + "' AND FG_OPERATION = 'Y' AND MASTERDATACLASSID = 'OperationItem'");
                        row["OPERATIONIDMAX"] = dttreeSet.Compute("MAX(USERSEQUENCE)", "PLANTID = '" + row["PLANTID"].ToString() + "' AND FG_OPERATION = 'Y' AND MASTERDATACLASSID = 'OperationItem'");
                    }
                    
                }
            }
            grdOpeMinMax.DataSource = dt;
        ...
        protected override void OnToolbarSaveClick()
        {
            ...
            if (dtRouting.Rows.Count != 0)
            {
                ...
                if (dataRowtree["MASTERDATACLASSID"].ToString() == "OperationItem")
                {
                    sASSEMBLYITEM = dataRowtree["PARENTS_ASSEMBLYITEMID"].ToString() + dataRowtree["PARENTS_ASSEMBLYITEMVERSION"].ToString();
                    
                }
                else
                {
                    sASSEMBLYITEM = dataRowtree["ASSEMBLYITEMID"].ToString() + dataRowtree["ASSEMBLYITEMVERSION"].ToString();
                }
            ...
            if (dtOperationup.Rows.Count != 0)
            {
                ...
                if (dataRowtree["ASSEMBLYROUTINGID"].ToString() == "")
                {
                    ...
                    rowRoutingComp["ENTERPRISEID"] = dataRowtree["ENTERPRISEID"];
                    rowRoutingComp["PLANTID"] = dataRowtree["PLANTID"];
                    rowRoutingComp["NEWREQUESTNO"] = "";
                    rowRoutingComp["ASSEMBLYITEMID"] = dataRowtree["ASSEMBLYITEMID"];
                    rowRoutingComp["ASSEMBLYITEMVERSION"] = dataRowtree["ASSEMBLYITEMVERSION"];
                    rowRoutingComp["ASSEMBLYITEMCLASS"] = dataRowtree["MASTERDATACLASSID"];
                    ...
                    rowBom["ENTERPRISEID"] = dataRowtree["ENTERPRISEID"].ToString();
                    rowBom["PLANTID"] = dataRowtree["PLANTID"].ToString();
                    rowBom["ASSEMBLYITEMID"] = dataRowtree["ASSEMBLYITEMID"].ToString();
                    rowBom["ASSEMBLYITEMVERSION"] = dataRowtree["ASSEMBLYITEMVERSION"].ToString();
                    rowBom["ASSEMBLYITEMUOM"] = dataRowtree["UOMDEFID"];
                    rowBom["ASSEMBLYITEMCLASS"] = dataRowtree["MASTERDATACLASSID"].ToString();
                    rowBom["ASSEMBLYTYPE"] = dataRowtree["MASTERDATACLASSID"].ToString();
                    rowBom["_STATE_"] = "added";
                    rowBom["VALIDSTATE"] = "Valid";
                    dtBom.Rows.Add(rowBom);
                    ...
                    if(dataRowtree["MASTERDATACLASSID"].ToString() == "SubAssembly")
                    {
                        Dictionary<string, object> parampf = new Dictionary<string, object>();
                        parampf.Add("ENTERPRISEID", dataRowtree["ENTERPRISEID"].ToString());
                        parampf.Add("PROCESSDEFID", dataRowtree["ASSEMBLYITEMID"].ToString());
                        parampf.Add("PROCESSDEFVERSION", dataRowtree["ASSEMBLYITEMVERSION"].ToString());
                        ...
                        if(dtBomChk != null)
                        {
                            if (dtBomChk.Rows.Count == 0)
                            {
                                ...
                                rowPROCESSDEFINITION["PROCESSDEFID"] = dataRowtree["ASSEMBLYITEMID"];
                                rowPROCESSDEFINITION["PROCESSDEFVERSION"] = dataRowtree["ASSEMBLYITEMVERSION"];
                                rowPROCESSDEFINITION["PROCESSCLASSID"] = "Semiproduct"; // 반제품
                                rowPROCESSDEFINITION["PROCESSDEFNAME"] = dataRowtree["ASSEMBLYITEMNAME"]; //
                                rowPROCESSDEFINITION["ENTERPRISEID"] = dataRowtree["ENTERPRISEID"];
                                rowPROCESSDEFINITION["PLANTID"] = dataRowtree["PLANTID"];
                                rowPROCESSDEFINITION["PROCESSDEFTYPE"] = "Main";
                                rowPROCESSDEFINITION["LEADTIME"] = dataRowtree["LEADTIME"]; //
                                rowPROCESSDEFINITION["VERSIONSTATE"] = "Active";
                                rowPROCESSDEFINITION["VALIDSTATE"] = "Valid";
                                rowPROCESSDEFINITION["_STATE_"] = "added";
                ...
                foreach (DataRow dataRow in  dtOperationup.Rows)
                {
                    ...
                    if (dataRowtree["MASTERDATACLASSID"].ToString() == "OperationItem")
                    {
                        ...
                        if(dataRow["_STATE_"].ToString() == "added")
                        {
                            rowPROCESSPATH["PROCESSPATHID"] = dataRow["OPERATIONID"];
                        }
                        else
                        {
                            if(dataRow["PROCESSSEGMENTID_OLD"].ToString() != "")
                            {

                                Dictionary<string, object> ParamProcesspath = new Dictionary<string, object>();
                                ParamProcesspath.Add("ENTERPRISEID", UserInfo.Current.Enterprise);
                               
                                
                                ParamProcesspath.Add("PROCESSDEFID", dataRowtree["PARENTS_ASSEMBLYITEMID"].ToString());
                                ParamProcesspath.Add("PROCESSDEFVERSION", dataRowtree["PARENTS_ASSEMBLYITEMVERSION"].ToString());
                                
                                

                                ParamProcesspath.Add("PROCESSSEGMENTID", dataRow["PROCESSSEGMENTID_OLD"]);
                                DataTable dProcesspath = SqlExecuter.Query("GetRoutingProcesspathList", "10001", ParamProcesspath);


                                if (dProcesspath != null)
                                {
                                    if (dProcesspath.Rows.Count != 0)
                                    {
                                        if (dataRow["OPERATIONID"].ToString() != dProcesspath.Rows[0]["PROCESSPATHID"].ToString())
                                        {
                                            rowPROCESSPATH["PROCESSPATHID"] = dProcesspath.Rows[0]["PROCESSPATHID"].ToString();
                                        }
                                        else
                                        {
                                            rowPROCESSPATH["PROCESSPATHID"] = dataRow["OPERATIONID"];
                        ...
                        rowPROCESSPATH["PROCESSDEFID"] = values["ITEMID"];
                        rowPROCESSPATH["PROCESSDEFVERSION"] = values["ITEMVERSION"];
                        ...
                        rowPROCESSPATH["DESCRIPTION"] = dataRow["DESCRIPTION"];

                        rowPROCESSPATH["VALIDSTATE"] = dataRow["VALIDSTATE"];
                        rowPROCESSPATH["_STATE_"] = dataRow["_STATE_"];

                        rowPROCESSPATH["PROCESSUOM"] = dataRow["PROCESSUOM"];
                    ...
                    else
                    {
                        ...
                        else
                        {
                            Dictionary<string, object> ParamProcesspath = new Dictionary<string, object>();
                            ParamProcesspath.Add("ENTERPRISEID", UserInfo.Current.Enterprise);
                            ParamProcesspath.Add("PROCESSDEFID", dataRowtree["ASSEMBLYITEMID"]);
                            ParamProcesspath.Add("PROCESSDEFVERSION", dataRowtree["ASSEMBLYITEMVERSION"]);
                        ...
                        rowPROCESSPATH["ENTERPRISEID"] = dataRow["ENTERPRISEID"];
                        rowPROCESSPATH["PLANTID"] = dataRow["PLANTID"];
                        rowPROCESSPATH["PROCESSDEFID"] = dataRowtree["ASSEMBLYITEMID"];
                        rowPROCESSPATH["PROCESSDEFVERSION"] = dataRowtree["ASSEMBLYITEMVERSION"];
                ...
                if (dataRowtree["MASTERDATACLASSID"].ToString() == "OperationItem")
                {
                    ParamStartend.Add("PROCESSDEFID", dataRowtree["PARENTS_ASSEMBLYITEMID"]);
                    ParamStartend.Add("PROCESSDEFVERSION", dataRowtree["PARENTS_ASSEMBLYITEMVERSION"]);
                }
                else
                {
                    ParamStartend.Add("PROCESSDEFID", dataRowtree["ASSEMBLYITEMID"]);
                    ParamStartend.Add("PROCESSDEFVERSION", dataRowtree["ASSEMBLYITEMVERSION"]);
                }
            ...
            if (dtBomComp.Rows.Count != 0)
            {
                ...
                foreach (DataRow rowBomComp in dtBomComp.Rows)
                {
                    ...
                    if (dataRowtree["MASTERDATACLASSID"].ToString() == "OperationItem")
                    {
                        ...
                        rowBILLOFMATERIAL["PRODUCTDEFID"] = dataRowtree["PARENTS_ASSEMBLYITEMID"];
                        rowBILLOFMATERIAL["PRODUCTDEFVERSION"] = dataRowtree["PARENTS_ASSEMBLYITEMVERSION"];
                        rowBILLOFMATERIAL["PROCESSDEFID"] = dataRowtree["PARENTS_ASSEMBLYITEMID"];
                        rowBILLOFMATERIAL["PROCESSDEFVERSION"] = dataRowtree["PARENTS_ASSEMBLYITEMVERSION"];
                    }
                    else
                    {
                        if (rowBomComp["COMPONENTITEMCLASS"].ToString() == "SubAssembly")
                        ...
                        rowBILLOFMATERIAL["PRODUCTDEFID"] = dataRowtree["ASSEMBLYITEMID"];
                        rowBILLOFMATERIAL["PRODUCTDEFVERSION"] = dataRowtree["ASSEMBLYITEMVERSION"];
                        rowBILLOFMATERIAL["PROCESSDEFID"] = dataRowtree["ASSEMBLYITEMID"];
                        rowBILLOFMATERIAL["PROCESSDEFVERSION"] = dataRowtree["ASSEMBLYITEMVERSION"];
                    }
                ...
                foreach (DataRow rowTool in dtTool.Rows)
                {
                    ...
                    if(rowTool["_STATE_"].ToString() == "modified")
                    {
                        if(rowTool["RESOURCEID"] != rowTool["RESOURCEIDOLD"])
                        {
                            ...
                            if (dataRowtree["MASTERDATACLASSID"].ToString() == "OperationItem")
                            {
                                rowBr["PRODUCTDEFID"] = dataRowtree["PARENTS_ASSEMBLYITEMID"];
                                rowBr["PRODUCTDEFVERSION"] = dataRowtree["PARENTS_ASSEMBLYITEMVERSION"];
                                rowBr["PROCESSDEFID"] = dataRowtree["PARENTS_ASSEMBLYITEMID"];
                                rowBr["PROCESSDEFVERSION"] = dataRowtree["PARENTS_ASSEMBLYITEMVERSION"];
                            }
                            else
                            {
                                rowBr["PRODUCTDEFID"] = dataRowtree["ASSEMBLYITEMID"];
                                rowBr["PRODUCTDEFVERSION"] = dataRowtree["ASSEMBLYITEMVERSION"];
                                rowBr["PROCESSDEFID"] = dataRowtree["ASSEMBLYITEMID"];
                                rowBr["PROCESSDEFVERSION"] = dataRowtree["ASSEMBLYITEMVERSION"];
                            }
                            ...
                            if (dataRowtree["MASTERDATACLASSID"].ToString() == "OperationItem")
                            {
                                rowBrD["PRODUCTDEFID"] = dataRowtree["PARENTS_ASSEMBLYITEMID"];
                                rowBrD["PRODUCTDEFVERSION"] = dataRowtree["PARENTS_ASSEMBLYITEMVERSION"];
                                rowBrD["PROCESSDEFID"] = dataRowtree["PARENTS_ASSEMBLYITEMID"];
                                rowBrD["PROCESSDEFVERSION"] = dataRowtree["PARENTS_ASSEMBLYITEMVERSION"];
                            }
                            else
                            {
                                rowBrD["PRODUCTDEFID"] = dataRowtree["ASSEMBLYITEMID"];
                                rowBrD["PRODUCTDEFVERSION"] = dataRowtree["ASSEMBLYITEMVERSION"];
                                rowBrD["PROCESSDEFID"] = dataRowtree["ASSEMBLYITEMID"];
                                rowBrD["PROCESSDEFVERSION"] = dataRowtree["ASSEMBLYITEMVERSION"];
                            }
        ...
        void fnSearch()
        {
            ...
            DataTable dtOpeMinMax = (DataTable)grdOpeMinMax.DataSource;
            if (dtOpeMinMax != null)
            {
                dtOpeMinMax.Clear();
            }
            ...
            //Dictionary<string, object> Param = new Dictionary<string, object>();
            //Param.Add("ITEMCODE", row["ITEMCODE"].ToString());
            ...
            //ParamRouting.Add("ENTERPRISEID", UserInfo.Current.Enterprise);
            //ParamRouting.Add("ASSEMBLYITEMID", row["ITEMCODE"].ToString());
            //ParamRouting.Add("ASSEMBLYITEMVERSION", row["ITEMVERSION"].ToString());
        ...
        private Micube.Framework.SmartControls.Validations.ValidationResultCommon ValidationProcesssegMnetPopup(DataRow currentGridRow, IEnumerable<DataRow> popupSelections)
		{
            ...
            switch(focusRow["MASTERDATACLASSID"].ToString())
            {
                ...
                case "":
                    ...
                    break;
                default:
                    param.Add("PARENTS_ASSEMBLYITEMID", focusRow["PARENTS_ASSEMBLYITEMID"]);
                    param.Add("PARENTS_ASSEMBLYITEMVERSION", focusRow["PARENTS_ASSEMBLYITEMVERSION"]);
                    break;
            }

            param.Add("ASSEMBLYITEMID", "");
            param.Add("ASSEMBLYITEMVERSION", "");
            ...
            DataTable dtOperationlist = SqlExecuter.Query("GetOperationlist", "10001", param);
