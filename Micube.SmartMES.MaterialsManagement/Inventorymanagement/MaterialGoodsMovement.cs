...
namespace Micube.SmartMES.MaterialsManagement
{
    ...
    /// 
    /// </summary>
    public partial class MaterialGoodsMovement : SmartConditionManualBaseForm
    {
        ...
        private void InitializeGrid()
        {
           ...
           // grdMaterial.View.SetAutoFillColumn("MATREMARK");
            ...
            grdMaterial.View.SetKeyColumn("PLANTID", "ENTERPRISEID", "CONSUMABLEDEFID", "CONSUMABLELOTID", "TOAREAID");
        ...
        /// </summary>
        private void InitializeGrid_ToAreaidPopup()
        {
            var popupGridToAreaid = this.grdMaterial.View.AddSelectPopupColumn("TOAREAID", new SqlQuery("GetAreaidListByCsm", "10001", $"P_ENTERPRISEID={UserInfo.Current.Enterprise}", $"LANGUAGETYPE={UserInfo.Current.LanguageType}"))
                // 팝업 폼 타이틀(다국어 키 입력), 버튼 유형, 검색 버튼 활성화 여부, 자동 검색 여부
                .SetPopupLayout("TOAREANAME", PopupButtonStyles.Ok_Cancel, true, false)
                // 팝업에서 전달해주는 결과값 개수 설정 (1 = 단일, 0 = 복수)
                .SetPopupResultCount(1)
                // 그리드의 컬럼과 팝업의 컬럼이 필드명이 다른 경우 맵핑 정보 설정 (그리드 컬럼 필드명, 팝업 컬럼 필드명)
                .SetPopupResultMapping("TOAREAID", "AREAID")
                // 팝업 폼 Layout 설정 (넓이, 높이, Border Style, 조회조건 구성 방향)
                ...
                .SetPopupApplySelection((selectedRows, dataGridRow) =>
                {
                    ...
                    foreach (DataRow row in selectedRows)
                    {
                        classRow["TOAREANAME"] = row["AREANAME"];
                        classRow["TOWAREHOUSEID"] = row["WAREHOUSEID"];
            ...
            ;
            // 팝업에서 사용할 조회조건 항목 추가 
            popupGridToAreaid.Conditions.AddTextBox("P_AREANAME")
                  .SetLabel("AREANAME"); 
            ...
            // 팝업 그리드 설정
            popupGridToAreaid.GridColumns.AddTextBoxColumn("AREAID", 120);
            popupGridToAreaid.GridColumns.AddTextBoxColumn("AREANAME", 200);
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < grdMaterial.View.DataRowCount; i++)
            {
                ...
                string strToAreaid = grdMaterial.View.GetRowCellValue(i, "TOAREAID").ToString();
                if (strToAreaid.Equals(""))
                {
                    string lblConsumabledefid = grdMaterial.View.Columns["TOAREAID"].Caption.ToString();
                    this.ShowMessage(MessageBoxButtons.OK, "InValidOspRequiredField", lblConsumabledefid); //메세지
                    return;
                }
                string strTowarehouseid = grdMaterial.View.GetRowCellValue(i, "TOWAREHOUSEID").ToString();
                if (strTowarehouseid.Equals(""))
                {
            ...
            if (result == System.Windows.Forms.DialogResult.Yes)
            {
                try
                {
                    ...
                    for (int irow = 0; irow < dtSavechang.Rows.Count; irow++)
                    {
                        ...
                        string ToAreaid = dr["TOAREAID"].ToString();
                        
                        ToAreaid = ToAreaid.Replace(",", "");
                        string[] arr = ToAreaid.Split('.');
                        dr["TOAREAID"] = arr[0];
                        

                        string ToWarehouseid = dr["TOWAREHOUSEID"].ToString();
                        ToWarehouseid = ToWarehouseid.Replace(",", "");
                        string[] arrwar = ToWarehouseid.Split('.');
                        dr["TOWAREHOUSEID"] = arrwar[0];
                        dr["TRANSACTIONDATE"] = dtTrans.ToString("yyyy-MM-dd HH:mm:ss");
                        dtSave.ImportRow(dr);
